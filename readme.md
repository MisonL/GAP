...
- 创建一个新的 Space。
- 将本项目代码上传到 Space。
- 在 Space 的 `Settings` -> `Secrets` 中设置以下环境变量：
...
1. **（重要）扩展 `app/data/model_limits.json`**: 确保此文件存在，并且为需要进行上下文截断的模型添加了 `"input_token_limit"` 字段（参考 Google 官方文档）。
2. 确保 `requirements.txt` 文件已包含必要的依赖 (`jinja2`, `starlette[full]` 等)。
3. Space 将会自动构建并运行。
4. URL格式为`https://your-space-url.hf.space`。

### 💻 本地运行

- 克隆项目代码到本地。

- 安装依赖：`pip install -r requirements.txt` (确保包含 `python-multipart` 等以支持所有功能)
- **（重要）创建并扩展模型限制文件：** 在 `app/data/` 目录下创建 `model_limits.json` 文件。定义模型的 RPM, RPD, TPD_Input, TPM_Input 限制，并且**为需要进行上下文截断的模型添加 `"input_token_limit"` 字段**（参考 Google 官方文档）。示例：
...
- 在连接中选择 OpenAI。
- 在 API Base URL 中填入 `https://your-space-url.hf.space/v1` 或本地运行地址 `http://localhost:7860/v1`。
- 在 API Key 中填入你的认证凭证：
...
本项目提供两套 API 接口：`/v1` (与 OpenAI API 兼容) 和 `/v2` (Gemini 原生 API)。

### OpenAI 兼容接口 (`/v1`)

这套接口旨在最大程度地兼容 OpenAI 的 Chat Completion API，方便接入现有的 OpenAI 生态工具和服务。
...
这套接口旨在高保真地代理 Gemini 原生 API 的功能，提供更直接的访问方式。
...
**Web UI 认证 (`/`, `/manage/context`, `/manage/keys`, `/report`):**
