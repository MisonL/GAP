...
if _context_storage_mode_env not in ['database', 'memory']:
    ...
    CONTEXT_STORAGE_MODE: str = "memory"
    ...
    CONTEXT_STORAGE_MODE: str = _context_storage_mode_env
...
if _key_storage_mode_env not in ['database', 'memory']:
    ...
    KEY_STORAGE_MODE: str = "memory" # 使用默认值
else:
    KEY_STORAGE_MODE: str = _key_storage_mode_env # 使用环境变量值
...
MODEL_LIMITS: Dict[str, Any] = {} # 初始化为空字典，将在应用启动时加载
    ...
    try:
        ...
        if os.path.exists(model_limits_path):
            ...
            with open(model_limits_path, 'r', encoding='utf-8') as f:
                MODEL_LIMITS = json.load(f) # 解析 JSON 数据并赋值给全局变量
            logger.info(f"成功加载模型限制。找到的模型: {list(MODEL_LIMITS.keys())}") # 记录成功日志和加载的模型列表
            # --- 可选：验证加载的数据结构 ---
            for model, limits in MODEL_LIMITS.items():
             ...
             MODEL_LIMITS = {} # 使用空字典作为默认值
    except json.JSONDecodeError as e: # 捕获 JSON 解析错误
        logger.error(f"解析模型限制文件 {model_limits_path} 失败: {e}。将使用空限制。") # 记录错误
        MODEL_LIMITS = {} # 使用空字典
