...
# --- 应用版本 ---
__version__ = "1.7.0" # 更新版本号到 v1.7.0，包含代码优化和重构内容
...
ENABLE_CONTEXT_COMPLETION: bool = os.environ.get("ENABLE_CONTEXT_COMPLETION", "true").lower() == "true" # 全局默认是否启用上下文补全
...
def load_model_limits():
    ...
    logger.info(f"尝试从 {model_limits_path} 加载模型限制...") # 尝试从 model_limits.json 加载模型限制
    try:
        if os.path.exists(model_limits_path):
            ...
            logger.info(f"成功加载模型限制。找到的模型: {list(MODEL_LIMITS.keys())}") # 成功加载模型限制并记录找到的模型
            # 验证加载的数据
            for model, limits in MODEL_LIMITS.items():
                 if not isinstance(limits, dict): # 检查限制是否为字典
                     logger.warning(f"模型 '{model}' 的限制不是字典格式，已忽略。") # 记录警告：模型限制不是字典格式
                     continue
                 if "input_token_limit" not in limits: # 检查是否缺少 'input_token_limit'
                      logger.warning(f"模型 '{model}' 在 model_limits.json 中缺少 'input_token_limit' 字段。") # 记录警告：缺少 'input_token_limit' 字段
                 elif limits["input_token_limit"] is None: # 检查 'input_token_limit' 是否为 null
                      logger.warning(f"模型 '{model}' 在 model_limits.json 中的 'input_token_limit' 为 null。") # 记录警告：'input_token_limit' 为 null
        else:
             logger.error(f"模型限制文件未找到: {model_limits_path}。将使用空限制。") # 记录错误：模型限制文件未找到
             MODEL_LIMITS = {} # 文件未找到，使用空字典
    except json.JSONDecodeError as e:
        logger.error(f"解析模型限制文件 {model_limits_path} 失败: {e}。将使用空限制。") # 记录错误：解析模型限制文件失败
        MODEL_LIMITS = {} # JSON 解析失败，使用空字典
    except Exception as e:
        logger.error(f"加载模型限制时发生未知错误: {e}", exc_info=True) # 记录未知错误：加载模型限制失败
