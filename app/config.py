...
DEFAULT_MAX_CONTEXT_TOKENS = int(os.environ.get('DEFAULT_MAX_CONTEXT_TOKENS', "30000"))
CONTEXT_TOKEN_SAFETY_MARGIN = int(os.environ.get('CONTEXT_TOKEN_SAFETY_MARGIN', "200"))
DEFAULT_CONTEXT_TTL_DAYS = 7 # 这个值主要用于 context_store 初始化数据库
# 新增：内存上下文清理运行间隔（秒），默认 1 小时
MEMORY_CONTEXT_CLEANUP_INTERVAL_SECONDS = int(os.environ.get('MEMORY_CONTEXT_CLEANUP_INTERVAL_SECONDS', "3600"))
# 新增：内存模式下最大上下文记录数，默认 5000
MAX_CONTEXT_RECORDS_MEMORY = int(os.environ.get('MAX_CONTEXT_RECORDS_MEMORY', "5000"))
    ...
    try:
        if os.path.exists(model_limits_path):
            ...
            for model, limits in MODEL_LIMITS.items():
                 ...
                 elif limits["input_token_limit"] is None:
