...
async def verify_proxy_key(request: Request) -> str:
    ...
    FastAPI dependency function to verify the Authorization header of API requests.
    - If using in-memory database (IS_MEMORY_DB=True), verifies if the Bearer token equals the PASSWORD environment variable.
    - If using file-based database (IS_MEMORY_DB=False), verifies if the Bearer token is a valid, active proxy key in the database.

    Args:
        request: FastAPI 请求对象。The FastAPI request object.

    Returns:
        str: 验证通过的令牌 (PASSWORD 或 代理 Key)。The validated token (PASSWORD or Proxy Key).

    Raises:
        HTTPException: 如果认证失败。If authentication fails.
    """
    auth_header = request.headers.get("Authorization") # 从请求头获取 Authorization 字段 (Get Authorization header from request headers)
    ...
    if IS_MEMORY_DB: # 检查当前是否为内存数据库模式 (Check if currently in memory database mode)
        ...
        return token # 返回验证通过的令牌 (Return the validated token)
    else:
        # --- 文件数据库模式：使用数据库中的代理 Key 验证 ---
        # --- File-based Database Mode: Verify using proxy keys from the database ---
        # 保持现有逻辑不变
        ...
        logger.debug(f"API 认证成功 (文件模式，使用代理 Key: {token[:8]}...)。") # Log successful authentication in file mode (DEBUG level)
