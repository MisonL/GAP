...
try:
    ...
    log_dir = os.path.join(tempfile.gettempdir(), 'gemini_api_proxy_logs') # 定义临时日志目录路径
    os.makedirs(log_dir, exist_ok=True) # 尝试创建临时日志目录
    print(f"警告: 无法在项目目录创建日志文件夹，将使用系统临时目录: {log_dir}") # 打印警告信息
except Exception as e:
    # 如果在项目根目录和临时目录创建都失败，则尝试在当前工作目录下创建 'logs' 文件夹
    log_dir = os.path.join(os.getcwd(), 'logs') # 定义当前工作目录下的日志目录路径
    try:
        os.makedirs(log_dir, exist_ok=True) # 尝试在当前工作目录创建
    except Exception as final_e:
        # 如果所有尝试都失败了，则将 log_dir 设置为 None，禁用文件日志记录
        log_dir = None # 将日志目录设为 None
        print(f"警告: 尝试在多个位置创建日志目录均失败: {final_e}。文件日志记录将被禁用。") # 打印最终警告信息
...
if log_dir: # 如果日志目录有效
    ...
    app_log_file = error_log_file = access_log_file = None # 将文件路径设为 None

# --- 日志格式配置 ---
# 从环境变量读取 DEBUG 模式设置，默认为 false
DEBUG = os.environ.get("DEBUG", "false").lower() == "true" # 获取 DEBUG 环境变量并转换为布尔值
# 定义 DEBUG 模式下的详细日志格式
LOG_FORMAT_DEBUG = '%(asctime)s - %(levelname)s - %(key_fmt)s%(request_type)s%(model_fmt)s%(status_code)s: %(message)s%(error_fmt)s' # DEBUG 模式日志格式
# 定义普通模式下的简洁日志格式
LOG_FORMAT_NORMAL = '%(key_fmt)s%(request_type)s%(model_fmt)s%(status_code)s: %(message)s' # 普通模式日志格式

# --- 日志轮转配置 ---
# 从环境变量读取单个日志文件最大大小 (MB)，转换为字节，默认 10MB
MAX_LOG_SIZE = int(os.environ.get("MAX_LOG_SIZE", "10")) * 1024 * 1024 # 计算最大日志文件大小
# 从环境变量读取保留的备份文件数量，默认 5 个
MAX_LOG_BACKUPS = int(os.environ.get("MAX_LOG_BACKUPS", "5")) # 获取最大备份文件数量
# 从环境变量读取日志轮转的时间间隔 (例如 'S', 'M', 'H', 'D', 'W0'-'W6', 'midnight')，默认 'midnight'
LOG_ROTATION_INTERVAL = os.environ.get("LOG_ROTATION_INTERVAL", "midnight") # 获取日志轮转间隔
# 从环境变量读取日志文件的最大保留天数，默认 30 天
LOG_CLEANUP_DAYS = int(os.environ.get("LOG_CLEANUP_DAYS", "30")) # 获取日志清理天数
...
def setup_logger():
    ...
    logger = logging.getLogger("my_logger") # 获取日志记录器
    # 根据 DEBUG 环境变量设置基础日志级别
    base_log_level = logging.DEBUG if DEBUG else logging.INFO # 根据 DEBUG 设置基础日志级别
    logger.setLevel(base_log_level) # 设置日志记录器级别
    print(f"日志基础级别设置为: {logging.getLevelName(base_log_level)}") # 添加启动时打印信息

    # 清除可能已存在的旧的处理程序，防止重复添加
    if logger.handlers: # 如果存在处理程序
        logger.handlers.clear() # 清除处理程序

    # --- 控制台处理程序 ---
    console_handler = logging.StreamHandler()  # 创建流处理程序（默认输出到 stderr）
    # 控制台日志的级别也应根据 DEBUG 模式调整，或者固定为 INFO
    console_log_level = logging.DEBUG if DEBUG else logging.INFO # 根据 DEBUG 设置控制台日志级别
    ...
    formatter = logging.Formatter('%(message)s') # 创建格式器
        ...
        try:
            ...
            if app_log_file: # 检查路径是否有效
                try:
                    ...
                    print(f"警告: 无法创建应用日志处理程序: {e}") # 打印警告信息
            ...
            if error_log_file: # 检查路径是否有效
                try:
                    ...
                    print(f"警告: 无法创建错误日志处理程序: {e}") # 打印警告信息

        except Exception as e:
            # 捕获设置文件日志处理程序时的任何其他异常
            print(f"警告: 设置文件日志处理程序时出错: {e}") # 打印警告信息
...
def format_log_message(level, message, extra=None):
    ...
    key = extra.get('key', '') # 获取 key
    request_type = extra.get('request_type', '') # 获取 request_type
    model = extra.get('model', '') # 获取 model
    status_code = extra.get('status_code', '') # 获取 status_code
    error_message = extra.get('error_message', '') # 获取 error_message
    ...
    log_format = LOG_FORMAT_DEBUG if DEBUG else LOG_FORMAT_NORMAL # 选择日志格式
    # 使用选定的格式和值来生成最终的日志字符串
    return log_format % log_values # 返回格式化后的字符串
...
def cleanup_old_logs(max_days=LOG_CLEANUP_DAYS):
    ...
    if not log_dir: # 如果日志目录无效
        print("信息: 日志目录无效，跳过日志清理。") # 打印信息
        return
    ...
    try:
        ...
        log_files = glob.glob(os.path.join(log_dir, '*.log*')) # 查找日志文件

        deleted_count = 0 # 初始化已删除文件的计数器
        # 遍历所有找到的日志文件（包括备份文件）
        for file_path in log_files: # 遍历文件路径
            try:
                ...
                # 计算文件的年龄（当前时间 - 最后修改时间）
                # 如果文件年龄超过了设定的最大保留秒数
                if now - file_mtime > max_age: # 如果文件已过期
                    try:
                        # 尝试删除这个过期的日志文件
                        ...
                        print(f"删除日志文件失败 {file_path}: {e}") # 打印删除失败信息
            except FileNotFoundError:
                 # 如果在处理过程中文件被删除（例如并发清理），则忽略
                 continue # 继续
            except Exception as e:
                # 如果在获取文件信息或比较时间时发生其他错误，打印错误信息
                print(f"处理日志文件时出错 {file_path}: {e}") # 打印处理文件错误信息

        # 清理完成后打印总结信息
        if deleted_count > 0: # 如果删除了文件
            print(f"日志清理完成，共删除 {deleted_count} 个超过 {max_days} 天的日志文件。") # 打印清理总结
        else:
            print(f"日志清理完成，没有找到超过 {max_days} 天的日志文件需要删除。") # 打印没有需要删除的文件信息
    except Exception as e:
        # 捕获在日志清理的顶层逻辑中发生的任何其他异常
        print(f"执行日志清理任务时发生错误: {e}") # 打印执行清理任务错误
