...
def report_usage(key_manager: 'APIKeyManager'):
    ...
    report_data: Dict[str, Any] = {
        "overall_stats": {
            "active_keys_count": active_keys_count, # 使用实际获取的活跃 Key 数量
            "invalid_keys_at_startup": INVALID_KEY_COUNT_AT_STARTUP,
            "rpd_capacity_estimation": [],
            "tpd_input_capacity_estimation": [],
            "current_rpd_today": 0,
            "current_tpd_input_today": 0,
            "estimated_rpd_today": "N/A",
            "estimated_tpd_input_today": "N/A",
            "estimation_fraction_of_day": 0,
            "historical_average_usage": {
        ...
        "key_suggestion": "正在加载建议...", # Default suggestion
        "top_ips": {
            "requests": {
                "today": [],
                "week": [],
    ...
    if not usage_data_copy:
        ...
        # 在没有 usage_data_copy 时，确保 key_usage_summary["models"] 是空列表
        ...
        if not key_status_summary:
            ...
            # 遍历并报告每个模型的 Key 使用情况
            report_data["key_usage_summary"]["models"] = [] # 确保初始化为列表
    ...
    target_rpd_capacity_total = 0 # 初始化目标模型的总 RPD 容量
    for target_model in target_models:
        ...
        if target_model_rpd_limit is not None:
            ...
            target_rpd_capacity_total += target_rpd_capacity # 累加目标模型的 RPD 容量
    ...
    for target_model in target_models:
        ...
        if target_model_tpd_input_limit is not None:
            ...
            target_tpd_input_capacity_total += target_tpd_input_capacity # 累加目标模型的 TPD 输入容量
    ...
    report_data["overall_stats"]["current_rpd_today"] = current_total_rpd # 报告今日已用 RPD
    report_data["overall_stats"]["current_tpd_input_today"] = current_total_tpd_input # 报告今日已用 TPD 输入
    ...
    # 平均 RPD
    # report_data["overall_stats"]["historical_average_usage"] = {} # 已在初始化时设置
    N = 7 # 统计过去 N 天
    ...
    tpd_input_usage_indicator = estimated_total_tpd_input_today if isinstance(estimated_total_tpd_input_today, (int, float)) else 0 # TPD 输入使用指标

    rpd_usage_ratio = 0
    if target_rpd_capacity_total > 0:
        rpd_usage_ratio = rpd_usage_indicator / target_rpd_capacity_total # 计算 RPD 使用率

    tpd_input_usage_ratio = 0
    if target_tpd_input_capacity_total > 0:
        tpd_input_usage_ratio = tpd_input_usage_indicator / target_tpd_input_capacity_total # 计算 TPD 输入使用率

    # 根据使用率提供 Key 数量建议
    if active_keys_count == 0:
        suggestion = "错误: 未找到有效的 API Key！"
    elif target_rpd_capacity_total <= 0 and target_tpd_input_capacity_total <= 0:
         suggestion = "无法生成建议 (目标模型 RPD/TPD_Input 限制未定义)。"
    elif target_rpd_capacity_total > 0 and rpd_usage_ratio > 0.85:
        # 如果 RPD 使用率过高，建议增加 Key 数量
        # 使用第一个目标模型的 RPD 限制来估算需要的 Key 数量
        first_target_model_rpd_limit = 0
        for target_model in target_models:
            target_model_limits = config.MODEL_LIMITS.get(target_model, {})
            if target_model_limits.get("rpd") is not None:
                 first_target_model_rpd_limit = target_model_limits["rpd"]
                 break # 找到第一个就退出循环

        if first_target_model_rpd_limit > 0:
             needed_keys = int(rpd_usage_indicator / (first_target_model_rpd_limit * 0.7)) # 计算建议的 Key 数量
             suggestion = f"警告: 预估/平均 RPD ({rpd_usage_indicator:,.0f}) 已达目标容量 ({target_rpd_capacity_total:,}) 的 {rpd_usage_ratio:.1%}。建议增加 Key 数量至约 {max(needed_keys, active_keys_count + 1)} 个。"
        else:
             suggestion = f"警告: 预估/平均 RPD ({rpd_usage_indicator:,.0f}) 已达目标容量 ({target_rpd_capacity_total:,}) 的 {rpd_usage_ratio:.1%}。无法估算建议的 Key 数量 (目标模型 RPD 限制未定义)。"
    elif target_tpd_input_capacity_total > 0 and tpd_input_usage_ratio > 0.85:
         # 如果 TPD 输入使用率过高，建议增加 Key 数量
         # 使用第一个目标模型的 TPD 输入限制来估算需要的 Key 数量
         first_target_model_tpd_input_limit = 0
         for target_model in target_models:
             target_model_limits = config.MODEL_LIMITS.get(target_model, {})
             if target_model_limits.get("tpd_input") is not None:
                  first_target_model_tpd_input_limit = target_model_limits["tpd_input"]
                  break # 找到第一个就退出循环

         if first_target_model_tpd_input_limit > 0:
              needed_keys = int(tpd_input_usage_indicator / (first_target_model_tpd_input_limit * 0.7)) # 计算建议的 Key 数量
              suggestion = f"警告: 预估 TPD 输入 ({tpd_input_usage_indicator:,.0f}) 已达目标容量 ({target_tpd_input_capacity_total:,}) 的 {tpd_input_usage_ratio:.1%}。建议增加 Key 数量至约 {max(needed_keys, active_keys_count + 1)} 个。"
         else:
              suggestion = f"警告: 预估 TPD 输入 ({tpd_input_usage_indicator:,.0f}) 已达目标容量 ({target_tpd_input_capacity_total:,}) 的 {tpd_input_usage_ratio:.1%}。无法估算建议的 Key 数量 (目标模型 TPD 输入限制未定义)。"
        ...
        if rpd_usage_ratio >= tpd_input_usage_ratio and target_rpd_capacity_total > 0:
             # 使用第一个目标模型的 RPD 限制来估算理想的 Key 数量
             first_target_model_rpd_limit = 0
             for target_model in target_models:
                 target_model_limits = config.MODEL_LIMITS.get(target_model, {})
                 if target_model_limits.get("rpd") is not None:
             ...
             if first_target_model_rpd_limit > 0:
                  ideal_keys = int(rpd_usage_indicator / (first_target_model_rpd_limit * 0.5)) + 1 # 计算理想的 Key 数量
                  suggestion = f"提示: 预估/平均 RPD ({rpd_usage_indicator:,.0f}) 较低 ({rpd_usage_ratio:.1%})。可考虑减少 Key 数量至约 {max(1, ideal_keys)} 个。"
             else:
                  suggestion = f"提示: 预估/平均 RPD ({rpd_usage_indicator:,.0f}) 较低 ({rpd_usage_ratio:.1%})。无法估算建议的 Key 数量 (目标模型 RPD 限制未定义)。"
        elif target_tpd_input_capacity_total > 0:
             # 使用第一个目标模型的 TPD 输入限制来估算理想的 Key 数量
             first_target_model_tpd_input_limit = 0
             for target_model in target_models:
                 target_model_limits = config.MODEL_LIMITS.get(target_model, {})
                 if target_model_limits.get("tpd_input") is not None:
             ...
             if first_target_model_tpd_input_limit > 0:
                  ideal_keys = int(tpd_input_usage_indicator / (first_target_model_tpd_input_limit * 0.5)) + 1 # 计算理想的 Key 数量
                  suggestion = f"提示: 预估 TPD 输入 ({tpd_input_usage_indicator:,.0f}) 较低 ({tpd_input_usage_ratio:.1%})。可考虑减少 Key 数量至约 {max(1, ideal_keys)} 个。"
             else:
                  suggestion = f"提示: 预估 TPD 输入 ({tpd_input_usage_indicator:,.0f}) 较低 ({tpd_input_usage_ratio:.1%})。无法估算建议的 Key 数量 (目标模型 TPD 输入限制未定义)。"

    report_data["key_suggestion"] = suggestion # 将建议添加到报告数据

    # --- Top 5 IP 地址统计 ---
    # report_data["top_ips"] = { ... } # 已在初始化时设置

    # 今日 Top 5 请求
    top_ips_today_req = get_top_ips(ip_counts_copy, today_pt.date(), today_pt.date())
    report_data["top_ips"]["requests"]["today"] = format_top_ips(top_ips_today_req, "count") # 修正 key_name 为 "count"

    # 今日 Top 5 输入 Token
    top_ips_today_input_token = get_top_ips(ip_input_token_counts_copy, today_pt.date(), today_pt.date())
    report_data["top_ips"]["tokens"]["today"] = format_top_ips(top_ips_today_input_token, "tokens") # 修正 key_name 为 "tokens"

    # 本周 Top 5 请求
    start_of_week_pt = today_pt - timedelta(days=today_pt.weekday()) # 计算本周开始日期
    top_ips_week_req = get_top_ips(ip_counts_copy, start_of_week_pt.date(), today_pt.date())
    report_data["top_ips"]["requests"]["week"] = format_top_ips(top_ips_week_req, "count") # 修正 key_name 为 "count"

    # 本周 Top 5 输入 Token
    top_ips_week_input_token = get_top_ips(ip_input_token_counts_copy, start_of_week_pt.date(), today_pt.date())
    report_data["top_ips"]["tokens"]["week"] = format_top_ips(top_ips_week_input_token, "tokens") # 修正 key_name 为 "tokens"

    # 本月 Top 5 请求
    start_of_month_pt = today_pt.replace(day=1) # 计算本月开始日期
    top_ips_month_req = get_top_ips(ip_counts_copy, start_of_month_pt.date(), today_pt.date())
    report_data["top_ips"]["requests"]["month"] = format_top_ips(top_ips_month_req, "count") # 修正 key_name 为 "count"
...
def format_top_ips(raw_data: List[Tuple[str, int]], key_name: str) -> List[Dict[str, Any]]:
    """
    将 Top IP 原始数据格式化为结构化字典列表。
    """
    # 确保返回的字典包含 'ip' 和 key_name 两个键
