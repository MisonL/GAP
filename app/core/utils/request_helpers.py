...
from fastapi import Request, HTTPException # 导入 FastAPI 的请求对象和 HTTP 异常类
from collections import defaultdict # 导入默认字典，方便初始化嵌套字典
from threading import Lock # 导入线程锁，用于保护共享数据
from typing import Dict, Union, List, Tuple # 导入类型提示
...
def protect_from_abuse(request: Request, max_rpm: int, max_rpd: int):
    ...
    """
    logger.warning("调用了可能已废弃的 protect_from_abuse 函数 (基于 IP 的速率限制)。") # 记录警告
    global ip_request_data, ip_daily_counts_lock, pacific_tz # 声明使用全局变量
    ...
    with ip_daily_counts_lock: # 获取 IP 计数锁，保证线程安全
        ...
        if len(timestamps) >= max_rpm:
            ...
            earliest_timestamp = timestamps[0] if timestamps else now # 获取窗口内最早的时间戳
            wait_time = max(0.0, earliest_timestamp + rpm_window_seconds - now) # 计算剩余等待时间，确保不为负
            logger.warning(f"IP {ip} 请求过于频繁，触发 RPM 限制 ({max_rpm} RPM)。需要等待 {wait_time:.2f} 秒。") # 记录警告
            # 抛出 429 错误
        ...
        logger.debug(f"IP {ip} 请求计数更新: RPD={ip_data['daily_count']}, RPM_Window_Count={len(timestamps)}") # 记录调试日志
