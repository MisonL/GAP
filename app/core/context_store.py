...
# 导入新的设置管理模块
from .db_settings import get_ttl_days, set_ttl_days # 注意：这些也需要变成 async
# 导入共享的数据库工具 和 新配置项
...
async def save_context(proxy_key: str, contents: List[Dict[str, Any]]):
    ...
    try:
        ...
        async with get_db_connection() as conn:
            ...
            await asyncio.to_thread(cursor.execute, "INSERT OR IGNORE INTO proxy_keys (key) VALUES (?)", (proxy_key,))
            ...
            if IS_MEMORY_DB and MAX_CONTEXT_RECORDS_MEMORY > 0:
                try:
                    # 获取当前记录数
                    await asyncio.to_thread(cursor.execute, "SELECT COUNT(*) FROM contexts")
                    ...
                    if current_count > MAX_CONTEXT_RECORDS_MEMORY:
                        ...
                        logger.info(f"成功删除了 {rowcount} 条最旧的内存上下文记录。")
...
async def load_context(proxy_key: str) -> Optional[List[Dict[str, Any]]]:
    ...
    try:
        async with get_db_connection() as conn:
            cursor = conn.cursor()
            await asyncio.to_thread(cursor.execute, "SELECT contents, last_used FROM contexts WHERE proxy_key = ?", (proxy_key,))
            row = await asyncio.to_thread(cursor.fetchone)
            ...
            if row and row['contents']:
                ...
                try:
                    ...
                    contents = await asyncio.to_thread(json.loads, row['contents'])
...
async def delete_context_for_key(proxy_key: str) -> bool:
    ...
    try:
        async with get_db_connection() as conn:
            cursor = conn.cursor()
            await asyncio.to_thread(cursor.execute, "DELETE FROM contexts WHERE proxy_key = ?", (proxy_key,))
            rowcount = cursor.rowcount # 获取同步操作的 rowcount
            await asyncio.to_thread(conn.commit)
            if rowcount > 0:
...
async def get_context_info(proxy_key: str) -> Optional[Dict[str, Any]]:
     ...
     try:
         async with get_db_connection() as conn:
             cursor = conn.cursor()
             await asyncio.to_thread(cursor.execute, "SELECT length(contents) as content_length, last_used FROM contexts WHERE proxy_key = ?", (proxy_key,))
             row = await asyncio.to_thread(cursor.fetchone)
             return dict(row) if row else None
     except sqlite3.Error as e:
...
async def cleanup_memory_context(max_age_seconds: int):
    ...
    try:
        async with get_db_connection() as conn:
            ...
            await asyncio.to_thread(cursor.execute, "DELETE FROM contexts WHERE last_used < ?", (cutoff_timestamp_str,))
            deleted_count = cursor.rowcount # 获取同步操作的 rowcount
            await asyncio.to_thread(conn.commit)
        if deleted_count > 0:
