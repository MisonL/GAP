...
from app.core.database.models import Base, UserKeyAssociation, KeyScore, Setting, ApiKey # 导入数据库模型
...
from app.core.tracking import key_scores_cache, cache_lock # 导入 Key 分数缓存和锁 (路径不变)
...
if use_memory_db_reason:
    ...
    # 文件数据库逻辑 (仅当上述条件都不满足时)
    logger.info(f"最终决定尝试使用文件数据库，因为所有内存条件评估为假。评估条件如下：HF Env: {IS_HF_ENV}, APP_DB_MODE env: '{os.getenv('APP_DB_MODE')}', config.KEY_STORAGE_MODE: '{app_config.KEY_STORAGE_MODE}'")
    _home_dir = os.path.expanduser("~") # 获取用户主目录
    _app_data_dir = os.path.join(_home_dir, '.gemini_api_proxy', 'data') # 定义应用数据目录路径
    try:
        os.makedirs(_app_data_dir, exist_ok=True) # 确保目录存在
        DATABASE_PATH = os.path.join(_app_data_dir, 'context_store.db') # 数据库文件路径
        logger.info(f"本地开发模式：数据库路径设置为 {_app_data_dir}")
    except PermissionError as e:
        logger.error(f"在创建本地数据库目录 {_app_data_dir} 时发生权限错误: {e}。将回退到内存数据库。")
        DATABASE_PATH = ":memory:" # 权限错误时回退到内存数据库
    except Exception as e:
        logger.error(f"创建本地数据库目录时发生未知错误: {e}。将回退到内存数据库。")
        DATABASE_PATH = ":memory:" # 其他错误时也回退到内存数据库
...
async def get_key_id_by_cached_content_id(db: AsyncSession, cached_content_id: str) -> Optional[int]:
    ...
    """
    logger.warning("get_key_id_by_cached_content_id 函数尚未完全实现，返回模拟数据。") # 记录警告
    try:
        # TODO: 实现从 CachedContent 表查询 key_id 的逻辑
        ...
        logger.error(f"根据缓存内容 ID '{cached_content_id}' 获取 Key ID 失败: {e}", exc_info=True) # 记录错误
        return None # 返回 None

async def get_key_string_by_id(db: AsyncSession, key_id: int) -> Optional[str]:
    """
    (需要实现) 根据 Key ID 获取 Key 字符串。
    目前是模拟实现。

    Args:
        db (AsyncSession): SQLAlchemy 异步数据库会话。
        key_id (int): 要查询的 Key ID。

    Returns:
        Optional[str]: Key 字符串，如果找不到或出错则返回 None。
    """
    logger.warning("get_key_string_by_id 函数尚未完全实现，返回模拟数据。") # 记录警告
    try:
        # TODO: 实现从 ApiKey 表查询 key_string 的逻辑
        # stmt = select(ApiKey.key_string).where(ApiKey.id == key_id)
        # result = await db.execute(stmt)
        # key_string = result.scalar_one_or_none()
        # return key_string
        return f"test_key_id_{key_id}" # 返回模拟数据
    except Exception as e:
        logger.error(f"根据 Key ID {key_id} 获取 Key 字符串失败: {e}", exc_info=True) # 记录错误
        return None # 返回 None

async def get_user_last_used_key_id(db: AsyncSession, user_id: str) -> Optional[int]:
    """
    (需要实现) 获取指定用户上次成功使用的 Key ID (用于粘性会话)。
    目前是模拟实现。
    ...
    Returns:
        Optional[int]: 用户上次使用的 Key ID，如果找不到或出错则返回 None。
    """
    logger.warning("get_user_last_used_key_id 函数尚未完全实现，返回模拟数据。") # 记录警告
    try:
        # TODO: 实现从 UserKeyAssociation 表查询 key_id 的逻辑，可能需要按 last_used_timestamp 排序
        ...
        logger.error(f"获取用户 '{user_id}' 上次成功使用的 Key ID 失败: {e}", exc_info=True) # 记录错误
        return None # 返回 None
