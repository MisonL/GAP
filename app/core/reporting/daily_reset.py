...
from app.core.dependencies import get_key_manager # 导入获取 Key 管理器的依赖函数
from app.core.tracking import ( # 从 tracking 模块导入共享数据和锁
    usage_data, usage_lock,             # Key 的实时使用数据和对应的锁
    daily_rpd_totals, daily_totals_lock # 存储每日总 RPD 的字典和对应的锁
)
    ...
    if key in usage_data and model in usage_data[key]:
        ...
        if "tpd_input_count" in usage_data[key][model]: # 检查是否存在 TPD_Input 计数器
            ...
            if usage_data[key][model].get("tpd_input_count", 0) > 0:
                 logger.debug(f"重置 TPD_Input 计数: Key={key[:8]}, Model={model}, TPD_In={usage_data[key][model].get('tpd_input_count', 0)} -> 0")
...
def _cleanup_daily_totals(pt_timezone: pytz.BaseTzInfo):
    ...
    with daily_totals_lock: # 确保持有锁
        ...
        # 如果删除了条目，记录日志
        if keys_to_delete:
            logger.info(f"已清理 {len(keys_to_delete)} 条旧的每日 RPD 总量记录 (早于 {cutoff_date})。")
...
async def reset_daily_counts(key_manager: APIKeyManager = Depends(get_key_manager)):
    ...
    logger.info(f"开始执行每日 RPD 和 TPD_Input 重置任务 (针对 PT 日期: {yesterday_date_str})...") # 记录任务开始日志
    ...
    with usage_lock: # 获取 usage_data 的锁
        ...
        for key in keys_to_reset:
            ...
            models_to_reset = list(usage_data.get(key, {}).keys()) # 使用 get 增加健壮性
            # 遍历每个模型
    ...
    try:
        ...
        logger.error(f"重置 Key Manager 每日配额耗尽标记时出错: {km_reset_err}", exc_info=True) # 记录错误

    logger.info(f"所有 Key 的 RPD 和 TPD_Input 计数已重置。") # 记录计数重置完成日志
    ...
    if total_rpd_yesterday > 0: # 如果昨天有 RPD 记录
        with daily_totals_lock: # 获取每日 RPD 总计的锁
            ...
            logger.info(f"记录 PT 日期 {yesterday_date_str} 的总 RPD: {total_rpd_yesterday}") # 记录日志
