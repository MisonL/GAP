...
try:
    from ..api.models import Message # 尝试从父级导入
except ImportError:
...
DATA_URI_REGEX = re.compile(r"^data:(" + "|".join(re.escape(m) for m in SUPPORTED_IMAGE_MIME_TYPES) + r");base64,(.+)$") # 编译 Data URI 正则表达式
...
def _process_text_content(
    ...
    if is_system_phase and role == 'system':
        ...
        return True, system_instruction_text # 标记为系统指令阶段，并返回更新后的系统指令文本
    else:
        ...
        if role in ['user', 'system']:  # 将 'system'（非首条）也视为 'user'
            ...
            return False, system_instruction_text # 标记为非系统指令阶段，并返回未更改的系统指令文本
...
def _process_multi_part_content(
    ...
    for item_index, item in enumerate(content):
        ...
        if item_type == 'text':
            ...
            if match: # 如果匹配成功 (If match is successful)
                ...
                error_msg = f"消息 {message_index} 项目 {item_index}: 无效或不支持的图像 Data URI。" # 错误消息 (Error message)
            ...
            errors.append(f"消息 {message_index} 项目 {item_index}: 不支持的内容类型 '{item_type}'") # 添加错误信息 (Add error message)
            has_error_in_item = True # 标记有错误 (Mark as having error)
    ...
    if parts and not has_error_in_item:
        ...
        # 如果当前多部分消息有错误，错误信息已记录在 errors 列表中
        return True # 标记为有错误
...
def convert_messages(messages: List[Message], use_system_prompt=False) -> Union[Tuple[List[Dict[str, Any]], Dict[str, Any]], List[str]]:
    ...
    for i, message in enumerate(messages):
        ...
        # 处理包含多部分的内容（例如文本和图像）
            ...
            # 遇到多部分内容时，退出系统指令处理阶段
            is_system_phase = False
            has_error_in_item = _process_multi_part_content(
                content,
                role,
                gemini_history,
                errors,
            ...
            # 如果多部分内容处理过程中有错误，继续下一个消息
            if has_error_in_item:
                continue
