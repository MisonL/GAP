...
from apscheduler.executors.asyncio import (  # 导入 APScheduler 的异步执行器
    AsyncIOExecutor,
)
from apscheduler.schedulers.asyncio import (  # 导入 APScheduler 的异步调度器
    AsyncIOScheduler,
)

# 从其他模块导入必要的组件
from gap import config  # 导入应用配置模块
from gap.config import CACHE_REFRESH_INTERVAL_SECONDS  # Key 分数缓存的刷新间隔（秒）
from gap.config import REPORT_LOG_LEVEL_INT  # 报告日志的级别 (整数形式)
from gap.config import USAGE_REPORT_INTERVAL_MINUTES  # 导入具体的配置项; 使用情况报告的间隔时间（分钟）
...
# 导入报告模块中的任务函数
from gap.core.reporting.daily_reset import (  # 每日计数重置函数 (新路径)
    reset_daily_counts,
)
...
def _format_key_usage_summary(models_summary: List[Dict[str, Any]]) -> List[str]:
    ...
    if not models_summary:  # 如果没有模型数据
        lines.append(
            f"  {COLOR_WARNING}暂无 Key 使用数据。{COLOR_RESET}"
        )  # 添加提示信息
    else:
        # 遍历每个模型的摘要信息
        for model_summary in models_summary:
            ...
            lines.append(
                f"  {COLOR_POSITIVE}模型: {model_summary.get('model_name', 'N/A')} (Key 数量: {model_summary.get('key_count', 0)}){COLOR_RESET}"
            )
            lines.append(
                f"    今日总 RPD: {model_summary.get('total_rpd_today', 0):,} | 今日总 TPD 输入: {model_summary.get('total_tpd_input_today', 0):,}"
            )
            lines.append("    状态分布:")  # 添加状态分布小标题
            status_distribution = model_summary.get(
                "status_distribution", []
            )  # 获取状态分布列表
            if not status_distribution:  # 如果没有状态分布数据
                lines.append(
                    f"      {COLOR_WARNING}无状态数据。{COLOR_RESET}"
                )  # 添加提示信息
            else:
                # 遍历每个状态及其对应的 Key 数量
                for status_info in status_distribution:
                    lines.append(
                        f"      - 数量: {status_info.get('count', 0)}, 状态: {status_info.get('status', 'N/A')}"
                    )
...
def _format_overall_stats(report_data: Dict[str, Any]) -> List[str]:
    ...
    key_selection_stats = report_data.get(
        "key_selection_stats", {}
    )  # 获取 Key 筛选统计数据
    ...
    lines.append(
        f"  启动时无效 Key 数量: {overall_stats.get('invalid_keys_at_startup', 0)}"
    )
    # 添加 RPD 容量估算
    ...
    if not rpd_capacity_estimations:
        ...
        for estimation in rpd_capacity_estimations:
            if "capacity" in estimation:
                lines.append(
                    f"    - 基于模型 {estimation.get('based_on', 'N/A')}: 限制 {estimation.get('limit', 'N/A')}, 容量 {estimation.get('capacity', 0):,}"
                )
            else:
                lines.append(
                    f"    - 基于模型 {estimation.get('based_on', 'N/A')}: {estimation.get('message', 'N/A')}"
                )
    # 添加 TPD 输入容量估算
    lines.append("  TPD 输入容量估算:")
    tpd_input_capacity_estimations = overall_stats.get(
    ...
    lines.append(
        f"  今日已用 TPD 输入: {overall_stats.get('current_tpd_input_today', 0):,}"
    )
    lines.append(
        f"  预估今日总 RPD: {overall_stats.get('estimated_rpd_today', 'N/A')} (基于已过去 {overall_stats.get('estimation_fraction_of_day', 0):.1%} 时间)"
    )
    lines.append(
        f"  预估今日总 TPD 输入: {overall_stats.get('estimated_tpd_input_today', 'N/A')} (基于已过去 {overall_stats.get('estimation_fraction_of_day', 0):.1%} 时间)"
    )
    # 添加历史平均 RPD
    historical_average_usage = overall_stats.get("historical_average_usage", {})
    lines.append(
        f"  历史平均每日 RPD ({historical_average_usage.get('days_included', 0)} 天): {historical_average_usage.get('avg_daily_rpd', 'N/A')}"
    )
    ...
    total_by_reason = key_selection_stats.get(
        "total_by_reason", {}
    )  # 获取按原因统计的总数
    if not total_by_reason:  # 如果没有数据
        lines.append(
            f"    {COLOR_WARNING}无 Key 筛选原因数据。{COLOR_RESET}"
        )  # 添加提示
    else:
        lines.append("  总计 (按原因):")  # 添加小标题
        # 按次数降序显示原因
        for reason, count in sorted(
            total_by_reason.items(), key=lambda item: item[1], reverse=True
        ):
...
def _format_key_suggestion(suggestion: str) -> List[str]:
    ...
    if "强烈建议" in suggestion or "接近或达到上限" in suggestion:
        ...
        lines.append(
            f"  {COLOR_WARNING}{suggestion}{COLOR_RESET}"
        )  # 普通增加建议用黄色
    elif "可以考虑减少" in suggestion:
        lines.append(
            f"  {COLOR_INFO}{suggestion}{COLOR_RESET}"
        )  # 减少建议用白色/默认色
    else:  # 保持当前数量的建议
...
def _format_top_ips(top_ips: Dict[str, Any]) -> List[str]:
    ...
    for period in ["today", "week", "month"]:
        ...
        ip_list = top_ips.get("requests", {}).get(
            period, []
        )  # 获取对应时间段的 IP 列表
        if not ip_list:  # 如果列表为空
            ...
            for ip_info in ip_list:
                lines.append(
                    f"      - {ip_info.get('ip', 'N/A')}: {ip_info.get('count', 0):,} 次请求"
                )  # 添加 IP 和请求次数
    ...
    for period in ["today", "week", "month"]:
        ...
        if not ip_list:  # 如果列表为空
            ...
            for ip_info in ip_list:
                lines.append(
                    f"      - {ip_info.get('ip', 'N/A')}: {ip_info.get('tokens', 0):,} Tokens"
                )  # 添加 IP 和 Token 数量
    return lines  # 返回格式化后的文本行列表
...
def log_usage_report(key_manager: "APIKeyManager"):
    ...
    try:
        ...
        report_lines = [
            f"{COLOR_TITLE}--- API 使用情况报告 ({report_data.get('timestamp', 'N/A')}) ---{COLOR_RESET}"
        ]  # 报告标题
        report_lines.extend(
            _format_key_usage_summary(
                report_data.get("key_usage_summary", {}).get("models", [])
            )
        )  # 格式化 Key 使用摘要
        report_lines.extend(
            _format_overall_stats(report_data)
        )  # 格式化总体统计、缓存、筛选
        report_lines.extend(
            _format_key_suggestion(report_data.get("key_suggestion", "无建议。"))
        )  # 格式化 Key 建议
        report_lines.extend(
            _format_top_ips(report_data.get("top_ips", {}))
        )  # 格式化 Top IP
        ...
        logger.error(
            f"记录周期性使用情况报告到终端失败: {e}", exc_info=True
        )  # 记录错误日志
...
def _add_memory_context_cleanup_job(context_store_manager: ContextStore):
    ...
    if config.CONTEXT_STORAGE_MODE == "memory":  # 检查是否为内存上下文模式
        ...
        logger.info(
            f"内存上下文清理任务 (ContextStore) 已添加，运行间隔: {cleanup_interval} 秒。"
        )
    else:
...
def setup_scheduler(key_manager: "APIKeyManager", context_store_manager: ContextStore):
    ...
    scheduler.add_job(
        log_usage_report,
        "interval",
        minutes=USAGE_REPORT_INTERVAL_MINUTES,
        args=[key_manager],
        id="usage_report",
        name="使用报告",
        replace_existing=True,
    ...
    scheduler.add_job(
        _refresh_all_key_scores,
        "interval",
        seconds=CACHE_REFRESH_INTERVAL_SECONDS,
        args=[key_manager],
        id="key_score_update",
        name="Key 得分更新",
        replace_existing=True,
