...
from datetime import date, datetime, timedelta, timezone  # 导入日期时间相关类
from typing import TYPE_CHECKING, Any, Dict, List, Tuple  # 导入类型提示

import pytz  # 导入时区库

from gap import config  # 导入应用配置模块
from gap.core.keys.checker import (  # 从 checker 模块导入启动时 Key 统计信息 (新路径)
    INITIAL_KEY_COUNT,
    INVALID_KEY_COUNT_AT_STARTUP,
)

# 从其他模块导入必要的组件
from gap.core.tracking import TPM_WINDOW_SECONDS  # Key 使用数据、锁、时间窗口常量
from gap.core.tracking import cache_lock  # Key 分数缓存和锁
from gap.core.tracking import cache_tracking_lock  # 缓存统计变量和锁
from gap.core.tracking import daily_totals_lock  # 每日 RPD 总计和锁
from gap.core.tracking import ip_input_token_counts_lock  # IP 每日输入 Token 计数和锁
...
# 从 request_helpers 导入 IP 请求数据
from gap.core.utils.request_helpers import ip_request_data  # (新路径)
...
# --- 辅助函数：获取 Top IPs ---
def get_top_ips(
    data_dict: Dict[str, Dict[str, int]], start_date: date, end_date: date, top_n=5
) -> List[Tuple[str, int]]:
    """
...
# --- 辅助函数：格式化 Top IPs ---
def format_top_ips(
    raw_data: List[Tuple[str, int]], key_name: str
) -> List[Dict[str, Any]]:
    """
...
def report_usage(key_manager: "APIKeyManager") -> Dict[str, Any]:  # 返回类型修改为 Dict
    ...
    start_of_week_pt = today_date - timedelta(
        days=today_pt.weekday()
    )  # 计算本周一的日期 (太平洋时区)
    ...
    try:
        ...
        with lock_manager.acquire_thread_lock("ip_rate_limit"):  # IP 请求计数锁
            ip_daily_request_counts_raw = copy.deepcopy(ip_request_data)  # 复制原始数据
            ip_request_data.clear()  # 清空 IP 请求数据
    except ImportError:
        # 如果锁管理器不可用，直接操作（虽然不完美但不会崩溃）
        ip_daily_request_counts_raw = copy.deepcopy(ip_request_data)  # 复制原始数据
        ip_request_data.clear()

    # 转换格式
    for ip, data in ip_daily_request_counts_raw.items():
        if isinstance(data, dict):
            # 获取计数信息
            daily_count = data.get("daily_count", 0)
            # 使用当前日期作为键
            today_str = (
                datetime.now(pytz.timezone("America/Los_Angeles")).date().isoformat()
            )
            ip_counts_copy[today_str][ip] = daily_count
    ...
    report_data: Dict[str, Any] = {
        ...
        "key_usage_summary": {"models": []},  # Key 使用情况摘要  # 按模型分类的摘要列表
        "key_suggestion": "正在加载建议...",  # Key 数量建议 (默认值)
        "top_ips": {  # Top IP 统计
            "requests": {"today": [], "week": [], "month": []},  # 按请求数统计
            "tokens": {"today": [], "week": [], "month": []},  # 按输入 Token 数统计
        },
    ...
    with cache_tracking_lock:  # 获取缓存跟踪锁
        ...
        if "cache_stats" not in report_data or not isinstance(
            report_data["cache_stats"], dict
        ):
    ...
    # 将统计结果添加到报告数据中
    # 防御性检查
    if "key_selection_stats" not in report_data or not isinstance(
        report_data["key_selection_stats"], dict
    ):
        report_data["key_selection_stats"] = {}
    # 存储按原因统计的总次数
    report_data["key_selection_stats"]["total_by_reason"] = dict(
    ...
    # --- 初始化用于聚合的字典 ---
    key_status_summary = defaultdict(
    ...
    # --- 构建报告文本行 (用于日志输出) ---
    report_lines = [
        f"{COLOR_TITLE}--- API 使用情况报告 ({today_pt.strftime('%Y-%m-%d %H:%M:%S %Z')}) ---{COLOR_RESET}"
    ]  # 报告标题
    ...
    # 防御性检查报告数据结构
    if "key_usage_summary" not in report_data or not isinstance(
        report_data["key_usage_summary"], dict
    ):
        report_data["key_usage_summary"] = {"models": []}
    if "models" not in report_data["key_usage_summary"] or not isinstance(
        report_data["key_usage_summary"]["models"], list
    ):
        report_data["key_usage_summary"]["models"] = []
    ...
    if not usage_data_copy:  # 如果没有使用数据
            ...
            for model_name, usage in models_usage.items():
                ...
                if rpm_limit is not None:  # 如果有 RPM 限制
                    if now - rpm_ts < RPM_WINDOW_SECONDS:  # 如果在窗口期内
                        ...
                        # 计算剩余百分比，避免除零错误
                        rpm_remaining_pct = (
                            max(0, (rpm_limit - rpm_in_window) / rpm_limit)
                            if rpm_limit > 0
                            else 0
                        )
                ...
                if tpm_input_limit is not None:  # 如果有 TPM 输入限制
                    if now - tpm_input_ts < TPM_WINDOW_SECONDS:  # 如果在窗口期内
                        ...
                        # 计算剩余百分比，避免除零错误
                        tpm_input_remaining_pct = (
                            max(
                                0,
                                (tpm_input_limit - tpm_input_in_window)
                                / tpm_input_limit,
                            )
                            if tpm_input_limit > 0
                            else 0
                        )

                # --- 计算 RPD 和 TPD 输入的剩余百分比 ---
                rpd_remaining_pct = (
                    max(0, (rpd_limit - rpd_count) / rpd_limit)
                    if rpd_limit is not None and rpd_limit > 0
                    else 1.0
                )
                tpd_input_remaining_pct = (
                    max(0, (tpd_input_limit - tpd_input_count) / tpd_input_limit)
                    if tpd_input_limit is not None and tpd_input_limit > 0
                    else 1.0
                )

                # 获取 Key 对该模型的分数 (如果存在)
                score = key_scores_cache_copy.get(key, {}).get(
                    model_name, -1.0
                )  # 默认 -1.0 表示无分数
        ...
        if not key_status_summary:  # 如果处理后仍然没有状态摘要
            ...
            for model_name, statuses in sorted(key_status_summary.items()):
                ...
                report_lines.append(
                    (
                        f"\n  {COLOR_POSITIVE}{model_name}{COLOR_RESET} (今日总 RPD: "
                        f"{model_total_rpd[model_name]:,}, 今日总 TPD 输入: "
                        f"{model_total_tpd_input[model_name]:,}, Key 数量: "
                        f"{total_keys_for_model})"
                    )
                )
                # 按 Key 数量降序遍历该模型下的状态
                for status, count in sorted(
                    statuses.items(), key=lambda item: item[1], reverse=True
                ):
                    # 添加状态详情到报告文本行
    ...
    # 防御性检查报告数据结构
    if "overall_stats" not in report_data or not isinstance(
        report_data["overall_stats"], dict
    ):
        report_data["overall_stats"] = {}
    if "rpd_capacity_estimation" not in report_data["overall_stats"] or not isinstance(
        report_data["overall_stats"]["rpd_capacity_estimation"], list
    ):
        report_data["overall_stats"]["rpd_capacity_estimation"] = []
    if "tpd_input_capacity_estimation" not in report_data[
        "overall_stats"
    ] or not isinstance(
        report_data["overall_stats"]["tpd_input_capacity_estimation"], list
    ):
        report_data["overall_stats"]["tpd_input_capacity_estimation"] = []
    if "historical_average_usage" not in report_data["overall_stats"] or not isinstance(
        report_data["overall_stats"]["historical_average_usage"], dict
    ):
        report_data["overall_stats"]["historical_average_usage"] = {
            "avg_daily_rpd": "N/A (无历史数据)",
            "days_included": 0,
        }
    ...
    report_lines.append(
        f"\n  {COLOR_INFO}RPD 容量估算 (基于 {active_keys_count} 个活跃 Key):{COLOR_RESET}"
    )  # 添加子标题
    # 遍历目标模型
    for target_model in target_models:
        target_model_limits = config.MODEL_LIMITS.get(
            target_model, {}
        )  # 获取目标模型的限制
        ...
        if target_model_rpd_limit is not None:  # 如果存在 RPD 限制
            ...
            # 添加到报告文本行
            report_lines.append(
                f"    - 基于 {COLOR_POSITIVE}{target_model}{COLOR_RESET} (RPD={target_model_rpd_limit}): {COLOR_POSITIVE}{target_rpd_capacity:,}{COLOR_RESET} 请求/天"
            )
            # 添加到报告数据字典
            ...
            logger.debug(
                f"目标模型 {target_model} 或其 RPD 限制未在 model_limits.json 中找到，无法估算 RPD 容量。"
            )  # 记录警告改为 debug
            report_lines.append(
                f"    - 基于 {COLOR_WARNING}{target_model}{COLOR_RESET}: {COLOR_WARNING}RPD 限制未定义{COLOR_RESET}"
            )  # 添加到报告文本行
            report_data["overall_stats"]["rpd_capacity_estimation"].append(
                {"based_on": target_model, "message": "RPD 限制未定义。"}
            )
    ...
    for rpd_limit, models in sorted(rpd_groups.items()):  # 按 RPD 限制值排序遍历
        if rpd_limit not in reported_rpd_limits:  # 如果此限制值未被报告过
            ...
            # 找出该组中实际被使用过的模型
            used_models_in_group = [
                m for m in models if model_rpd_usage_count.get(m, 0) > 0
            ]
            if used_models_in_group:  # 如果该组有模型被使用过
                # 添加到报告文本行
                report_lines.append(
                    f"    - 基于 {COLOR_POSITIVE}{', '.join(used_models_in_group)}{COLOR_RESET} (RPD={rpd_limit}): {COLOR_POSITIVE}{group_capacity:,}{COLOR_RESET} 请求/天"
                )
                # 添加到报告数据字典
                report_data["overall_stats"]["rpd_capacity_estimation"].append(
                    {
    ...
    report_lines.append(
        f"\n  {COLOR_INFO}TPD 输入容量估算 (基于 {active_keys_count} 个活跃 Key):{COLOR_RESET}"
    )  # 添加子标题
    # 遍历目标模型
    for target_model in target_models:
        target_model_limits = config.MODEL_LIMITS.get(
        ...
        target_model_tpd_input_limit = target_model_limits.get(
            "tpd_input"
        )  # 获取 TPD 输入限制值
        if target_model_tpd_input_limit is not None:  # 如果存在 TPD 输入限制
            ...
            report_lines.append(
                (
                    f"    - 基于 {COLOR_POSITIVE}{target_model}{COLOR_RESET} "
                    f"(TPD_In={target_model_tpd_input_limit:,}): "
                    f"{COLOR_POSITIVE}{target_tpd_input_capacity:,}{COLOR_RESET} Token/天"
                )
            )
            # 添加到报告数据字典
            ...
            report_lines.append(
                f"    - 基于 {COLOR_WARNING}{target_model}{COLOR_RESET}: {COLOR_WARNING}TPD 输入限制未定义{COLOR_RESET}"
            )  # 添加到报告文本行
            report_data["overall_stats"]["tpd_input_capacity_estimation"].append(
                {"based_on": target_model, "message": "TPD 输入限制未定义。"}
            )

    # --- 今日用量与全天预估 ---
    current_total_rpd = sum(model_total_rpd.values())  # 计算今日所有模型的总 RPD
    current_total_tpd_input = sum(
        model_total_tpd_input.values()
    )  # 计算今日所有模型的总 TPD 输入
    # 更新报告数据
    ...
    report_lines.append(
        f"    - 总 RPD: {COLOR_POSITIVE}{current_total_rpd:,}{COLOR_RESET}"
    )
    report_lines.append(
        f"    - 总 TPD 输入: {COLOR_POSITIVE}{current_total_tpd_input:,}{COLOR_RESET}"
    )

    # 计算今天已经过去的时间占全天的比例 (太平洋时区)
    seconds_since_pt_midnight = (
        today_pt - today_pt.replace(hour=0, minute=0, second=0, microsecond=0)
    ).total_seconds()
    fraction_of_day_passed = (
        seconds_since_pt_midnight / (24 * 3600) if seconds_since_pt_midnight > 0 else 0
    )
    estimated_total_rpd_today = 0  # 初始化预估 RPD
    estimated_total_tpd_input_today = 0  # 初始化预估 TPD 输入

    report_lines.append(
        f"\n  {COLOR_INFO}今日 ({today_date_str}) 预估全天用量 (基于已过去 {fraction_of_day_passed:.1%}):{COLOR_RESET}"
    )  # 添加子标题
    ...
    if fraction_of_day_passed > 0.01:
        ...
        estimated_total_tpd_input_today = int(
            current_total_tpd_input / fraction_of_day_passed
        )
        # 更新报告数据
        report_data["overall_stats"]["estimated_rpd_today"] = estimated_total_rpd_today
        report_data["overall_stats"]["estimation_fraction_of_day"] = round(
            fraction_of_day_passed, 3
        )
        # 添加到报告文本行
        report_lines.append(
            f"    - 预估总 RPD: {COLOR_POSITIVE}{estimated_total_rpd_today:,}{COLOR_RESET}"
        )
        # 对 TPD 输入估算设置更高的时间阈值 (例如 10%)，因为 Token 量波动可能更大
        if fraction_of_day_passed > 0.1:
            report_data["overall_stats"][
                "estimated_tpd_input_today"
            ] = estimated_total_tpd_input_today
            report_lines.append(
                f"    - 预估总 TPD 输入: {COLOR_POSITIVE}{estimated_total_tpd_input_today:,}{COLOR_RESET}"
            )
        else:  # 时间过早，不显示 TPD 估算
            report_data["overall_stats"]["estimated_tpd_input_today"] = "N/A (时间过早)"
            report_lines.append(
                f"    - 预估总 TPD 输入: {COLOR_WARNING}N/A (时间过早，估算不准确){COLOR_RESET}"
            )
    else:  # 时间过早，不进行估算
        ...
        report_lines.append(
            f"    - 预估总 RPD: {COLOR_WARNING}N/A (时间过早，估算不准确){COLOR_RESET}"
        )
        report_lines.append(
            f"    - 预估总 TPD 输入: {COLOR_WARNING}N/A (时间过早，估算不准确){COLOR_RESET}"
        )
    ...
    for i in range(1, N + 1):
        day_str_loop = (today_date - timedelta(days=i)).strftime(
            "%Y-%m-%d"
        )  # 计算日期字符串
    ...
    if last_n_days_rpd:  # 如果有历史数据
        ...
        # 更新报告数据
        report_data["overall_stats"]["historical_average_usage"]["avg_daily_rpd"] = (
            round(avg_daily_rpd, 0)
        )
        report_data["overall_stats"]["historical_average_usage"]["days_included"] = len(
            last_n_days_rpd
        )
        # 添加到报告文本行
        report_lines.append(
    ...
    rpd_usage_indicator = max(
        (
            estimated_total_rpd_today
            if isinstance(estimated_total_rpd_today, (int, float))
            else 0
        ),
        avg_daily_rpd if isinstance(avg_daily_rpd, (int, float)) else 0,
    )
    # 使用 TPD 输入指标：取今日预估 TPD 输入
    tpd_input_usage_indicator = (
        estimated_total_tpd_input_today
        if isinstance(estimated_total_tpd_input_today, (int, float))
        else 0
    )
    ...
    if target_tpd_input_capacity_total > 0:  # 避免除零错误
        tpd_input_usage_ratio = (
            tpd_input_usage_indicator / target_tpd_input_capacity_total
        )
    ...
    # 检查是否需要增加 Key (关键阈值)
    if (
        rpd_usage_ratio > RPD_CRITICAL_USAGE_THRESHOLD
        or tpd_input_usage_ratio > TPD_INPUT_CRITICAL_USAGE_THRESHOLD
    ):
        # 估算满足当前用量 + 20% 缓冲所需的 Key 数量
        needed_keys_rpd = (
            (rpd_usage_indicator / first_target_model_rpd_limit) * 1.2
            if first_target_model_rpd_limit > 0
            else active_keys_count * 2
        )  # 如果限制为0，则建议翻倍
        needed_keys_tpd_input = (
            (tpd_input_usage_indicator / first_target_model_tpd_input_limit) * 1.2
            if first_target_model_tpd_input_limit > 0
            else active_keys_count * 2
        )
        # 取两者所需的最大值，并确保至少增加 1 个
        suggested_key_count = max(
            active_keys_count + 1, int(max(needed_keys_rpd, needed_keys_tpd_input))
        )
    ...
    # 检查是否需要增加 Key (高阈值)
        ...
        rpd_usage_ratio > RPD_HIGH_USAGE_THRESHOLD
        or tpd_input_usage_ratio > TPD_INPUT_HIGH_USAGE_THRESHOLD
    ):
    ...
    # 检查是否可以减少 Key (低阈值，且当前 Key 数大于启动时的数量)
        ...
        rpd_usage_ratio < RPD_LOW_USAGE_THRESHOLD
        and tpd_input_usage_ratio < TPD_INPUT_LOW_USAGE_THRESHOLD
        and active_keys_count > INITIAL_KEY_COUNT
    ):
    ...
    report_data["top_ips"]["requests"]["today"] = format_top_ips(
        top_ips_today_req, "count"
    )  # 格式化并存入报告数据
    report_lines.append(
        f"    - 今日 ({today_date_str}): {top_ips_today_req if top_ips_today_req else 'N/A'}"
    )  # 添加到报告文本行
    # 本周 Top IP (请求数)
    top_ips_week_req = get_top_ips(ip_counts_copy, start_of_week_pt, today_date)
    report_data["top_ips"]["requests"]["week"] = format_top_ips(
        top_ips_week_req, "count"
    )
    report_lines.append(
        f"    - 本周 ({start_of_week_pt.strftime('%Y-%m-%d')} - {today_date_str}): {top_ips_week_req if top_ips_week_req else 'N/A'}"
    )
    # 本月 Top IP (请求数)
    top_ips_month_req = get_top_ips(ip_counts_copy, start_of_month_pt, today_date)
    report_data["top_ips"]["requests"]["month"] = format_top_ips(
        top_ips_month_req, "count"
    )
    report_lines.append(
        f"    - 本月 ({start_of_month_pt.strftime('%Y-%m-%d')} - {today_date_str}): {top_ips_month_req if top_ips_month_req else 'N/A'}"
    )
    ...
    # 今日 Top IP (Token 数)
    top_ips_today_token = get_top_ips(
        ip_input_token_counts_copy, today_date, today_date
    )
    report_data["top_ips"]["tokens"]["today"] = format_top_ips(
        top_ips_today_token, "tokens"
    )
    report_lines.append(
        f"    - 今日 ({today_date_str}): {top_ips_today_token if top_ips_today_token else 'N/A'}"
    )
    # 本周 Top IP (Token 数)
    top_ips_week_token = get_top_ips(
        ip_input_token_counts_copy, start_of_week_pt, today_date
    )
    report_data["top_ips"]["tokens"]["week"] = format_top_ips(
        top_ips_week_token, "tokens"
    )
    report_lines.append(
        f"    - 本周 ({start_of_week_pt.strftime('%Y-%m-%d')} - {today_date_str}): {top_ips_week_token if top_ips_week_token else 'N/A'}"
    )
    # 本月 Top IP (Token 数)
    top_ips_month_token = get_top_ips(
        ip_input_token_counts_copy, start_of_month_pt, today_date
    )
    report_data["top_ips"]["tokens"]["month"] = format_top_ips(
        top_ips_month_token, "tokens"
    )
    report_lines.append(
        f"    - 本月 ({start_of_month_pt.strftime('%Y-%m-%d')} - {today_date_str}): {top_ips_month_token if top_ips_month_token else 'N/A'}"
    )
    ...
    if selection_reason_counts:  # 如果有筛选记录
        report_lines.append(
            f"\n  {COLOR_INFO}总计 (按原因):{COLOR_RESET}"
        )  # 添加子标题
        # 按次数降序遍历原因统计
        for reason, count in sorted(
            selection_reason_counts.items(), key=lambda item: item[1], reverse=True
        ):
            report_lines.append(f"    - {reason}: {count} 次")  # 添加到报告文本行

        report_lines.append(
            f"\n  {COLOR_INFO}详情 (按 Key 和原因):{COLOR_RESET}"
        )  # 添加子标题
        # 按 Key 排序遍历详细统计
        ...
        report_lines.append(
            f"  {COLOR_WARNING}暂无 Key 筛选记录。{COLOR_RESET}"
        )  # 添加提示信息
    ...
    # 添加缓存统计信息到报告文本行
    report_lines.append(
        f"  {COLOR_INFO}缓存命中次数: {COLOR_POSITIVE}{report_data['cache_stats']['hit_count']:,}{COLOR_RESET}"
    )
    report_lines.append(
        f"  {COLOR_INFO}缓存未命中次数: {COLOR_WARNING}{report_data['cache_stats']['miss_count']:,}{COLOR_RESET}"
    )
    report_lines.append(
        f"  {COLOR_INFO}总共节省 Token 数: {COLOR_POSITIVE}{report_data['cache_stats']['total_tokens_saved']:,}{COLOR_RESET}"
    )
    report_lines.append(
        f"  {COLOR_INFO}缓存命中率: {COLOR_POSITIVE}{report_data['cache_stats']['hit_rate']}{COLOR_RESET}"
    )
