...
from datetime import datetime, timezone, timedelta # 导入 datetime 相关
...
# --- 锁，用于保护共享数据结构的并发访问 ---
timestamps_lock = asyncio.Lock() # 用于保护 ip_timestamps
daily_counts_lock = asyncio.Lock() # 用于保护 ip_daily_counts
...
async def protect_from_abuse(
    ...
    if max_requests_per_minute > 0: # 仅在配置了限制时检查
        async with timestamps_lock: # 获取锁以保护共享数据
    ...
    if max_requests_per_day_per_ip > 0: # 仅在配置了限制时检查
        async with daily_counts_lock: # 获取锁以保护共享数据
