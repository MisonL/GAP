...
from sqlalchemy import insert, select, update  # 导入 SQLAlchemy 相关函数和类
...
async def get_setting(
    ...
    logger.info(
        f"get_setting: Received db type: {type(db)}, db repr: {repr(db)}"
    )  # 添加日志
    """
...
async def get_ttl_days(db: AsyncSession) -> int:
    ...
    使用传入的 SQLAlchemy AsyncSession。
    """
    value_str = await get_setting(
        db, "context_ttl_days", str(config.DEFAULT_CONTEXT_TTL_DAYS)
    )
    try:
        val = int(value_str)
        return max(0, val)
        ...
        logger.warning(
            f"无效的 TTL 设置值 '{value_str}'，将使用默认值 {config.DEFAULT_CONTEXT_TTL_DAYS}"
        )
