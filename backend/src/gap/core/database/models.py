...
from sqlalchemy import (  # 导入列类型和函数
    Boolean,
    Column,
    DateTime,
    Float,
...
from sqlalchemy.orm import declarative_base, sessionmaker  # 导入声明性基类和会话创建器
...
class ApiKey(Base):
    ...
    )  # API Key 字符串，不允许为空，必须唯一，并建立索引以加快查询
    description = Column(String, nullable=True)  # Key 的描述信息，可以为空
    created_at = Column(
        DateTime(timezone=True), server_default=func.now()
    )  # Key 的创建时间，带时区信息，数据库自动设置为当前时间
    expires_at = Column(
        DateTime(timezone=True), nullable=True
    )  # Key 的过期时间，带时区信息，可以为空（表示永不过期）
    is_active = Column(
        Boolean, default=True, nullable=False
    )  # Key 是否处于激活状态，默认为 True，不允许为空
    enable_context_completion = Column(
        Boolean, default=True, nullable=False
    )  # 此 Key 是否启用上下文补全功能，默认为 True，不允许为空
    user_id = Column(
        String, nullable=True, index=True
    )  # 与此 Key 关联的用户 ID (可选)，可以为空，建立索引
    ...
    def __repr__(self):
        ...
        key_preview = (
            f"{self.key_string[:8]}..." if self.key_string else "None"
        )  # 获取 Key 的前 8 位作为预览
...
class UserKeyAssociation(Base):
    ...
    )  # 关联的 ApiKey 表的 ID，不允许为空，建立索引
    last_used_timestamp = Column(
        Float, nullable=False
    )  # 最后使用该 Key 的时间戳 (Unix timestamp)
...
class CachedContent(Base):
    ...
    content_id = Column(
        String, nullable=False, unique=True, index=True
    )  # 缓存内容的唯一 ID (通常是内容的哈希值)，不允许为空，唯一且索引
    ...
    )  # 创建此缓存时使用的 Key ID (可选)，建立索引
    creation_timestamp = Column(
        Float, nullable=False
    )  # 缓存创建时的时间戳 (Unix timestamp)
    expiration_timestamp = Column(
        Float, nullable=False
    )  # 缓存过期时的时间戳 (Unix timestamp)
    gemini_cache_id = Column(
        String, nullable=True, index=True
    )  # Gemini API 返回的缓存 ID (可选)
