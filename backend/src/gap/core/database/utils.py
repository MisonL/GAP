...
from sqlalchemy import delete, select, update  # 导入 SQLAlchemy 查询、更新、删除构造器
...
from gap.core.database.models import (  # 导入数据库模型
    ApiKey,
    Base,
    UserKeyAssociation,
)
from gap.core.tracking import (  # 导入 Key 分数缓存和锁 (路径不变)
    cache_lock,
    key_scores_cache,
)
...
if app_config.KEY_STORAGE_MODE == "memory":
    use_memory_db_reason.append(
        f"config.KEY_STORAGE_MODE is '{app_config.KEY_STORAGE_MODE}' (可能是默认值或环境变量设置)"
    )
...
if use_memory_db_reason:
    ...
    # 文件数据库逻辑
    logger.info(
        f"尝试使用文件数据库。内存条件评估: HF Env: {IS_HF_ENV}, APP_DB_MODE env: '{os.getenv('APP_DB_MODE')}', config.KEY_STORAGE_MODE: '{app_config.KEY_STORAGE_MODE}'"
    )
    ...
    if custom_db_path_from_env:
        logger.info(
            f"检测到环境变量 CONTEXT_DB_PATH 设置为: '{custom_db_path_from_env}'"
        )
        # 如果用户指定了路径，尝试使用它
        # 需要确保路径的目录存在
        ...
        if db_dir:  # 如果路径包含目录部分
            try:
                ...
                logger.error(
                    f"根据 CONTEXT_DB_PATH 创建目录 '{db_dir}' 时发生权限错误: {e}。将回退到默认路径或内存数据库。"
                )
                DATABASE_PATH = None  # 标记为 None，以便后续逻辑回退
            except Exception as e:
                logger.error(
                    f"根据 CONTEXT_DB_PATH 创建目录 '{db_dir}' 时发生未知错误: {e}。将回退到默认路径或内存数据库。"
                )
                DATABASE_PATH = None  # 标记为 None
        else:  # 如果路径只是文件名，则在当前工作目录创建 (或需要进一步定义行为)
            DATABASE_PATH = custom_db_path_from_env
            logger.info(
                f"使用 CONTEXT_DB_PATH 指定的数据库文件名 (将在当前目录创建): {DATABASE_PATH}"
            )
    ...
    if DATABASE_PATH is None:  # 如果 CONTEXT_DB_PATH 尝试失败或未设置
        ...
        try:
            ...
            logger.error(
                f"创建默认应用数据目录 '{_app_data_dir}' 时发生权限错误: {e}。将回退到内存数据库。"
            )
            DATABASE_PATH = ":memory:"
        except Exception as e:
            logger.error(
                f"创建默认应用数据目录时发生未知错误: {e}。将回退到内存数据库。"
            )
            DATABASE_PATH = ":memory:"
...
async def add_api_key(
    ...
    try:
        ...
        # 记录成功添加的日志
        logger.info(
            f"成功添加 API Key: {new_api_key.key_string[:8]}... (ID: {new_api_key.id})"
        )
        return new_api_key  # 返回创建的对象
    except (
        sqlalchemy.exc.IntegrityError
    ) as e:  # 捕获唯一约束冲突错误 (通常是 key_string 重复)
        await db.rollback()  # 回滚事务
        logger.error(
            f"添加 API Key 时发生唯一约束冲突 (Key 可能已存在): {key_string[:8]}... - {e}",
            exc_info=False,
        )  # 记录错误 (不包含堆栈)
        return None  # 返回 None
        ...
        await db.rollback()  # 回滚事务
        logger.error(
            f"添加 API Key {key_string[:8]}... 失败: {e}", exc_info=True
        )  # 记录详细错误日志
        return None  # 返回 None
...
async def get_api_key_by_string(db: AsyncSession, key_string: str) -> Optional[ApiKey]:
    ...
    """
    if not key_string:
        return None  # 如果 key_string 为空，直接返回 None
    try:
...
async def update_api_key(
    db: AsyncSession, key_string: str, updates: Dict[str, Any]
) -> Optional[ApiKey]:
    """
    ...
    """
    if not key_string or not updates:
        return None  # 如果 key_string 或 updates 为空，直接返回 None
    try:
        # 1. 先查询要更新的 Key 是否存在
        ...
        if "expires_at" in allowed_updates:
            ...
            if isinstance(expiry_value, datetime):  # 如果是 datetime 对象
                ...
                if expiry_value.tzinfo is None:
                    logger.warning(
                        f"更新 Key {key_string[:8]}... 的 expires_at 时提供了 naive datetime，假设为本地时间并转换为 UTC。建议提供 aware datetime。"
                    )
                    # 假设本地时区，转换为 UTC (这里可能需要更健壮的时区处理)
                    # 或者直接要求调用者提供 aware datetime
                    allowed_updates["expires_at"] = expiry_value.astimezone(
                        timezone.utc
                    )
                else:
                    allowed_updates["expires_at"] = expiry_value.astimezone(
                ...
                logger.warning(
                    f"更新 Key {key_string[:8]}... 时提供的 expires_at ('{expiry_value}') 不是有效的 datetime 对象或 None，已忽略此字段。"
                )  # 记录警告
        ...
        if not allowed_updates:
            logger.warning(
                f"没有有效的字段需要为 Key {key_string[:8]}... 更新。"
            )  # 记录警告
        ...
        logger.info(
            f"成功更新 API Key: {key_string[:8]}... 更新内容: {allowed_updates}"
        )  # 记录成功更新日志
        return api_key_to_update  # 返回更新后的对象
    except Exception as e:  # 捕获可能的数据库异常
        await db.rollback()  # 回滚事务
        logger.error(
            f"更新 API Key {key_string[:8]}... 失败: {e}", exc_info=True
        )  # 记录错误日志
        return None  # 返回 None
...
async def delete_api_key(db: AsyncSession, key_string: str) -> bool:
    ...
    """
    if not key_string:
        return False  # 如果 key_string 为空，返回 False
    try:
        # 构建 DELETE 语句
        ...
        if result.rowcount > 0:  # 如果删除了至少一行
            ...
            logger.warning(
                f"尝试删除 API Key: {key_string[:8]}... 时未找到匹配项。"
            )  # 记录警告
            return True  # 也可以认为操作成功完成（目标状态已达成）
    except Exception as e:  # 捕获可能的数据库异常
        await db.rollback()  # 回滚事务
        logger.error(
            f"删除 API Key {key_string[:8]}... 失败: {e}", exc_info=True
        )  # 记录错误日志
...
async def get_key_id_by_cached_content_id(
    db: AsyncSession, cached_content_id: str
) -> Optional[int]:
    """
    ...
    try:
        ...
        if isinstance(cached_content_id, int):  # 如果传入的是整数 ID
            stmt = select(CachedContent.key_id).where(
                CachedContent.id == cached_content_id
            )
        else:  # 否则按 content_id (字符串) 查询
            stmt = select(CachedContent.key_id).where(
                CachedContent.content_id == cached_content_id
            )
        ...
        if key_id is not None:
            logger.info(
                f"成功从缓存标识符 '{cached_content_id}' 获取到 key_id: {key_id}"
            )
        else:
        ...
        logger.error(
            f"根据缓存内容标识符 '{cached_content_id}' 获取 Key ID 失败: {e}",
            exc_info=True,
        )
        return None
...
async def get_key_string_by_id(db: AsyncSession, key_id: int) -> Optional[str]:
    ...
    try:
        ...
        if key_string:
            logger.info(
                f"成功为 Key ID {key_id} 获取到 Key 字符串: {key_string[:8]}..."
            )
        else:
...
async def get_user_last_used_key_id(db: AsyncSession, user_id: str) -> Optional[int]:
    ...
    try:
        stmt = (
            select(UserKeyAssociation.key_id)
        ...
        logger.error(
            f"获取用户 '{user_id}' 上次成功使用的 Key ID 失败: {e}", exc_info=True
        )
        return None
...
async def get_key_scores(model_name: str) -> Dict[str, float]:
    ...
    """
    logger.debug(
        "get_key_scores 函数可能依赖于内存缓存或未完全实现的数据库逻辑。"
    )  # 记录警告改为 debug
    try:
        # TODO: 实现从 KeyScore 表获取分数的逻辑，或者确认是否依赖内存缓存
        # 暂时从内存缓存获取
        with cache_lock:  # 使用锁保护缓存访问
            scores = key_scores_cache.get(
                model_name, {}
            )  # 获取模型对应的分数，默认为空字典
            return scores.copy()  # 返回缓存的副本
    except Exception as e:
        logger.error(
            f"获取模型 '{model_name}' 的 Key 分数失败: {e}", exc_info=True
        )  # 记录错误
        return {}  # 出错时返回空字典
...
async def update_setting(db: AsyncSession, key: str, value: str):
    ...
    """
    logger.warning(
        "调用了可能重复或已废弃的 update_setting 函数，建议使用 set_setting。"
    )  # 记录警告
    try:
        ...
        if result.rowcount == 0:  # 如果没有行被更新 (说明 key 不存在)
            ...
            logger.info(
                f"设置 '{key}' 已更新为 '{value}' (通过 update_setting)"
            )  # 记录更新日志
        await db.commit()  # 提交事务
    except Exception as e:
        logger.error(
            f"使用 update_setting 更新设置 '{key}' 时发生错误: {e}", exc_info=True
        )  # 记录错误日志
        await db.rollback()  # 回滚事务
