...
from typing import Any, Dict, Optional, Tuple  # 导入类型提示
...
def _format_api_error(http_err: httpx.HTTPStatusError) -> Tuple[str, int]:
    ...
    try:
        ...
        if (
            ...
            and "error" in error_detail
            and "message" in error_detail["error"]
        ):
            error_message = (
                f"API 错误 (状态码 {status_code}): {error_detail['error']['message']}"
            )
        else:
...
def _handle_429_daily_quota(
    ...
    try:
        ...
        if (
            error_detail
            and "error" in error_detail
            and "details" in error_detail["error"]
        ):
...
async def _handle_http_error_in_attempt(
    ...
    logger.error(
        f"API HTTP 错误 ({'流式' if is_stream else '非流式'}, Key: {current_api_key[:8] if current_api_key else 'N/A'}, Request: {request_id}): {status_code} - {error_message}",
        exc_info=False,
    )  # exc_info=False 避免重复记录堆栈
    ...
    if status_code in [500, 503]:  # 服务器内部错误或服务不可用
        error_info["type"] = (
            "server_error" if status_code == 500 else "service_unavailable_error"
        )
        needs_retry = True  # 这些通常是临时性问题，需要重试 (尝试其他 Key 或稍后重试)
        logger.warning(f"HTTP 状态码 {status_code} 表示服务器临时错误，标记需要重试。")
        # 标记 Key 临时不可用
        if current_api_key:
            key_manager.mark_key_temporarily_unavailable(
                current_api_key, duration_seconds=60, issue_type=f"HTTP {status_code}"
            )
        ...
        if _handle_429_daily_quota(http_err, current_api_key, key_manager):
            ...
            error_info["message"] = (
                f"Key {current_api_key[:8] if current_api_key else 'N/A'} 每日配额耗尽，尝试其他 Key。"
            )
        else:
            ...
            logger.warning(
                "HTTP 状态码 429 (非每日配额) 表示当前 Key 速率限制，标记无需重试。"
            )
            # 仍然标记 Key 临时不可用，以便 Key 选择器避开
            if current_api_key:
                key_manager.mark_key_temporarily_unavailable(
                    current_api_key, duration_seconds=60, issue_type="Rate Limit (429)"
                )

    elif status_code in [401, 403]:  # 未授权或禁止访问
        error_info["type"] = (
            "authentication_error" if status_code == 401 else "permission_error"
        )
        ...
        if current_api_key:
            key_manager.mark_key_temporarily_unavailable(
                ...
                issue_type=f"Auth/Permission ({status_code})",
            )
        ...
        if "API key not valid" in error_message:
            logger.warning(
                "HTTP 状态码 400 (明确 Key 无效) 表示 Key 无效，标记无需重试。"
            )
            ...
            if current_api_key:
                key_manager.mark_key_temporarily_unavailable(
                    current_api_key,
                    duration_seconds=300,
                    issue_type="Invalid API Key (400)",
                )
        else:
            logger.warning(
                "HTTP 状态码 400 (非 Key 无效) 表示请求体问题，标记无需重试。"
            )
...
async def _handle_api_call_exception(
    ...
    if isinstance(exc, httpx.HTTPStatusError):  # --- 处理 HTTP 状态错误 ---
        ...
        if current_api_key:
            key_manager.mark_key_temporarily_unavailable(
                current_api_key, duration_seconds=60, issue_type="Timeout"
            )
        ...
        if current_api_key:
            key_manager.mark_key_temporarily_unavailable(
                current_api_key, duration_seconds=60, issue_type="Connection Error"
            )
    else:  # --- 处理其他所有未预料到的异常 ---
