...
from typing import Any, AsyncGenerator, Dict, List, Optional  # 导入类型提示
...
# 导入需要在这里使用的工具函数
from gap.core.processing.utils import (  # 导入工具函数
    save_context_after_success,
)
    ...
    if not assistant_message_yielded:  # 检查是否从未生成过有效内容
        ...
        if actual_finish_reason == "STOP":  # 如果完成原因是正常停止 (STOP)
            if safety_issue_detail_received:
                ...
                logger.warning(
                    f"流 {response_id}: 结束时未产生助手内容，完成原因是 STOP，但检测到安全问题。向客户端发送安全提示。详情: {safety_issue_detail_received}"
                )  # 记录警告日志
                ...
                logger.error(
                    f"流 {response_id}: 结束时未产生助手内容，但完成原因是 STOP。向客户端发送错误。"
                )  # 记录错误日志
            ...
            logger.warning(
                f"流 {response_id}: 结束时未产生助手内容 (完成原因: {actual_finish_reason})。发送包含 finish_reason 的结束块。"
            )  # 记录警告日志
            ...
            end_chunk = {
                ...
                "choices": [
                    {"delta": {}, "index": 0, "finish_reason": actual_finish_reason}
                ],  # delta 为空，但包含 finish_reason
            }
        ...
        logger.debug(
            f"流 {response_id}: 正常结束，发送包含 finish_reason '{actual_finish_reason}' 的结束块。"
        )  # 记录调试日志
        end_chunk = {
            "id": response_id,
            "object": "chat.completion.chunk",
...
async def generate_stream_response(
    ...
    contents: List[
        Dict[str, Any]
    ],  # 经过处理和可能截断后，要发送给 Gemini API 的内容列表
    safety_settings: List[Dict[str, Any]],  # 安全设置列表
    system_instruction: Optional[Dict[str, Any]],  # 系统指令（Gemini system_instruction 字典）
    ...
    content_to_cache_on_success: Optional[
        Dict[str, Any]
    ],  # 如果是缓存未命中，成功后需要缓存的原始内容（通常是请求体）
    ...
    assistant_message_yielded = (
        False  # 标记是否已生成并发送了有效的助手消息（文本或工具调用）
    )
    ...
    try:
        ...
        async for chunk_data in gemini_client_instance.stream_chat(
            ...
            if isinstance(chunk_data, dict):  # 如果是字典类型的数据块
                ...
                if "_usage_metadata" in chunk_data:  # 使用量元数据
                    usage_metadata_received = chunk_data[
                        "_usage_metadata"
                    ]  # 存储元数据
                    logger.debug(
                        f"流 {response_id}: 接收到 usage metadata: {usage_metadata_received}"
                    )  # 记录日志
                    continue  # 元数据不直接发送给客户端，继续处理下一个块
                elif "_final_finish_reason" in chunk_data:  # 最终完成原因
                    actual_finish_reason = chunk_data[
                        "_final_finish_reason"
                    ]  # 更新实际完成原因
                    logger.debug(
                        f"流 {response_id}: 接收到最终完成原因: {actual_finish_reason}"
                    )  # 记录日志
                    continue  # 完成原因在流结束后统一处理，继续
                elif "_safety_issue" in chunk_data:  # 安全问题详情
                    safety_issue_detail_received = chunk_data[
                        "_safety_issue"
                    ]  # 存储安全问题详情
                    logger.warning(
                        f"流 {response_id}: 接收到安全问题详情: {safety_issue_detail_received}"
                    )  # 记录警告日志
                    ...
                    logger.info(
                        f"流 {response_id}: 接收到工具调用: {final_tool_calls}"
                    )  # 记录日志
                ...
                # 处理未知类型的块（基于当前 GeminiClient 返回类型定义，此分支理论上不会触发）
                logger.warning(  # type: ignore[unreachable]
                    f"流 {response_id}: 接收到未知类型的块: {type(chunk_data)}"
                )  # 记录警告日志
        ...
        if not stream_error_occurred:  # 确保流处理过程中没有发生错误
            ...
            if assistant_message_yielded or final_tool_calls:
                ...
                if usage_metadata_received:
                    prompt_tokens = usage_metadata_received.get(
                        "promptTokenCount"
                    )
                    logger.debug(
                        "流 %s: 请求成功，接收到 usage metadata，promptTokenCount=%s (当前仅记录，不更新配额计数)。",
                        response_id,
                        prompt_tokens,
                    )
                else:
                    # 如果没有收到使用量元数据，记录警告
                    logger.warning(
                        f"流 {response_id}: 响应成功但未找到 usage metadata。Token 计数未更新。"
                    )  # 记录警告
                ...
                with usage_lock:  # 使用锁保证线程安全
                    ...
                    key_usage = usage_data.setdefault(
                        selected_key, defaultdict(lambda: defaultdict(int))
                    )[model_name]
                    key_usage["last_used_timestamp"] = time.time()  # 更新时间戳
                    logger.debug(
                        f"流 {response_id}: 请求成功，更新 Key {selected_key[:8]}... ({model_name}) 的 last_used_timestamp"
                    )  # 记录日志
                ...
                if user_id_for_mapping and db_for_cache:  # 确保有 user_id 和 db session
                    try:
                        ...
                        await key_manager.update_user_key_association(
                            db_for_cache, user_id_for_mapping, selected_key
                        )
                        logger.debug(
                            f"流 {response_id}: 请求成功，更新用户 {user_id_for_mapping} 与 Key {selected_key[:8]}... 的关联。"
                        )  # 记录日志
                    except Exception as assoc_err:
                        # 记录更新关联失败的错误
                        logger.error(
                            f"流 {response_id}: 更新用户 Key 关联失败: {assoc_err}",
                            exc_info=True,
                        )
                elif user_id_for_mapping and not db_for_cache:
                    # 如果有用户 ID 但没有数据库会话，记录警告
                    logger.warning(
                        f"流 {response_id}: db session 无效，跳过用户 Key 关联更新。"
                    )  # 记录警告

                # 4. 创建缓存 (如果启用了原生缓存、是缓存未命中且成功生成了内容)
                if enable_native_caching and content_to_cache_on_success:
                    logger.debug(
                        f"流 {response_id}: 请求成功且是缓存未命中，尝试创建新缓存 (Key: {selected_key[:8]}...)"
                    )  # 记录日志
                    try:
                        # 确保数据库会话和用户 ID 有效
                        if db_for_cache and user_id_for_mapping is not None:
                            ...
                            if api_key_id is not None:  # 确保成功获取到 ID
                                ...
                                if new_cache_id:  # 如果成功创建缓存
                                    logger.info(
                                        f"流 {response_id}: 新缓存创建成功: {new_cache_id} (Key: {selected_key[:8]}...)"
                                    )  # 记录成功日志
                                    # TODO: 实现 Key 与缓存的关联更新逻辑 (在 key_manager 中)
                                else:  # 如果创建失败
                                    logger.warning(
                                        f"流 {response_id}: 创建新缓存失败 (Key: {selected_key[:8]}...)"
                                    )  # 记录失败警告
                            else:  # 如果无法获取 Key ID
                                logger.warning(
                                    f"流 {response_id}: 无法获取 Key {selected_key[:8]}... 的 ID，跳过缓存创建。"
                                )  # 记录警告
                        else:  # 如果数据库会话或用户 ID 无效
                            logger.warning(
                                f"流 {response_id}: db session 或 user_id 无效，跳过缓存创建。"
                            )  # 记录警告
                    except Exception as cache_create_err:
                        # 捕获并记录缓存创建过程中可能发生的异常
                        logger.error(
                            f"流 {response_id}: 创建缓存时发生异常 (Key: {selected_key[:8]}...): {cache_create_err}",
                            exc_info=True,
                        )  # 记录错误
                ...
                if (
                    config.STREAM_SAVE_REPLY
                    and user_id_for_mapping
                    and (full_reply_content or final_tool_calls)
                ):
                    logger.info(
                        "流 %s: STREAM_SAVE_REPLY 已启用，准备保存流式响应上下文。",
                        response_id,
                    )
                    if not db_for_cache:
                        logger.warning(
                            "流 %s: STREAM_SAVE_REPLY 已启用但缺少数据库会话，跳过上下文保存。",
                            response_id,
                        )
                    else:
                        try:
                    ...
                    has_text = "有" if full_reply_content else "无"
                    has_tools = "有" if final_tool_calls else "无"
                    logger.debug(
                        "流 %s: STREAM_SAVE_REPLY 已启用，但无内容 (%s 文本, %s 工具调用) "
                        "或无 user_id_for_mapping (%s)，跳过上下文保存。",
                        response_id,
                        has_text,
                        has_tools,
                        user_id_for_mapping,
                    ...
                    logger.debug(
                        "流 %s: STREAM_SAVE_REPLY 未启用，跳过流式响应上下文保存。",
                        response_id,
                    )
        ...
        # --- 处理 API 调用时的 HTTP 错误 ---
        logger.error(
            f"流 {response_id}: API HTTP 错误: {http_err.response.status_code} - {http_err.response.text}",
            exc_info=False,
        )  # 记录错误日志
        stream_error_occurred = True  # 标记发生错误
        # 格式化错误信息 (需要 _format_api_error 函数，此处简化)
        error_info = {
            "message": f"API Error: {http_err.response.status_code}",
            "type": "api_error",
        ...
        # --- 处理流处理过程中的其他意外异常 ---
        logger.error(
            f"流 {response_id}: 处理中捕获到意外异常: {stream_e}", exc_info=True
        )  # 记录详细错误日志
        stream_error_occurred = True  # 标记发生错误
        # 构造通用内部错误信息
        error_info = {
