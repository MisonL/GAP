...
from typing import Dict, Any, Optional, List, Tuple, Set, Union # 导入类型提示
...
from gap.core.tracking import (
    ...
    CACHE_REFRESH_INTERVAL_SECONDS # 常量：RPM/TPM 窗口秒数和缓存刷新间隔秒数
)
...
class APIKeyManager:
    ...
    def __init__(self):
        ...
        self.session_hidden_web_ui_keys: Set[str] = set() # 存储在当前会话中被“虚拟删除”的 WEB_UI_PASSWORDS
        self.session_env_key_configs: Dict[str, Dict[str, Any]] = {} # 存储环境变量Key在当前会话中的临时配置
        ...
        if config.KEY_STORAGE_MODE == 'memory': # --- 内存模式 ---
            ...
            if config.ADMIN_API_KEY:
                ...
                if admin_key and admin_key not in new_api_keys:
                    ...
                    logger.info(f"已添加管理员 API Key 到有效密钥列表。")
    ...
    async def select_best_key(self, model_name: str, model_limits: Dict[str, Any], estimated_input_tokens: int,
        ...
        with self.keys_lock:
            ...
            user_association_reason = "User Assoc. - Skipped" # 初始化 user_association_reason
                            ...
                            if associated_key_str not in current_active_keys: # 是否在当前活动 Key 列表中
                                ...
                                with usage_lock: # 获取使用数据锁
                                    ...
                                    if tpm_input_limit is None or tpm_input_limit <= 0 or potential_tpm_input <= tpm_input_limit:
                                        ...
                                        # 计算剩余可用输入 Token 容量
                                        available_input_tokens = max(0, tpm_input_limit - tpm_input_used) if tpm_input_limit is not None and tpm_input_limit > 0 else 10**18
                                        ...
                                        return selected_key, int(available_input_tokens) # 返回选定的 Key 和可用 Token 容量
                                    else: # --- Token 预检查失败 ---
                            ...
                            if last_used_key_str not in current_active_keys: # 是否在当前活动 Key 列表中
                                ...
                                with usage_lock: # 获取使用数据锁
                                    ...
                                    if tpm_input_limit is None or tpm_input_limit <= 0 or potential_tpm_input <= tpm_input_limit:
                                        ...
                                        return selected_key, int(available_input_tokens) # 返回结果
                                    else: # --- Token 预检查失败 ---
            ...
            if selected_key is None: # 如果经过前两种策略仍未选定 Key
                ...
                if not scores: # 如果没有分数数据
                    ...
                    if not available_scores: # 如果筛选后没有可用的 Key
                        ...
                        best_score: float = sorted_keys_by_score[0][1] if sorted_keys_by_score else 0.0
                        # 筛选出分数在阈值范围内的 Key
                        keys_in_rotation_range: List[Tuple[str, float]] = [
                            (k, score) for k, score in sorted_keys_by_score
    ...
    async def get_api_key_for_cached_content(
        ...
        cached_content_identifier: Union[int, str] # 可以是 CachedContent.id 或 CachedContent.content_id
        ...
        from gap.core.database.models import CachedContent, ApiKey # 延迟导入模型
        ...
        try:
            ...
            if config.KEY_STORAGE_MODE == 'database':
                ...
                with self.keys_lock: # 保护对 key_configs 的访问
                    # 添加类型注解
