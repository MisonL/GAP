...
async def test_api_key(api_key: str, http_client: httpx.AsyncClient) -> bool:
    ...
    try:
        # 使用共享的 http_client 发送异步 GET 请求，设置超时时间为 10 秒
            ...
            try:
                ...
                if "models" in data and isinstance(data["models"], list):
                    ...
                    return True # 返回 True
                     ...
                     logger.warning(f"测试 Key {api_key[:10]}... 成功 (状态码 200)，但响应 JSON 格式不符合预期: {data}") # 记录警告日志
                     return False # 返回 False
            except json.JSONDecodeError: # 捕获 JSON 解析错误
                 # 如果状态码是 200 但无法解析响应体，记录警告并视为无效
                 logger.warning(f"测试 Key {api_key[:10]}... 成功 (状态码 200)，但无法解析响应体为 JSON。") # 记录警告日志
                 return False # 返回 False
        else: # --- 状态码非 200 ---
    ...
    except httpx.RequestError as e: # 捕获其他 httpx 请求相关的错误 (如 DNS 解析、连接错误等)
        # 处理网络连接错误等请求相关错误
        logger.warning(f"测试 Key {api_key[:10]}... 时发生网络请求错误: {e}") # 记录警告日志
        return False # 网络错误视为无效

    except Exception as e: # 捕获其他所有未预料到的异常
        # 捕获并记录测试过程中发生的任何其他未知错误
        logger.error(f"测试 Key {api_key[:10]}... 时发生未知错误: {e}", exc_info=True) # 记录错误日志，包含堆栈信息
