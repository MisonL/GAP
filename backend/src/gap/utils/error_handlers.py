...
from gap.core.utils.request_helpers import (  # 导入获取客户端 IP 的函数 (新路径)
    get_client_ip,
)
...
def translate_error(message: str) -> str:
    ...
    # --- 添加翻译规则 ---
    if "quota exceeded" in lower_message:
        return "API 密钥配额已用尽或达到速率限制"
    if "api key not valid" in lower_message:
        return "提供的 API 密钥无效"
    if "invalid argument" in lower_message:
        return "请求参数无效或格式错误"
    if "internal server error" in lower_message:
        return "Gemini API 服务器内部错误，请稍后重试"
    if "service unavailable" in lower_message:
        return "Gemini API 服务暂时不可用，请稍后重试"
    if "permission denied" in lower_message:
        return "无权访问所请求的资源或 API"
    if "cancelled" in lower_message:
        return "操作被取消"
    if "deadline exceeded" in lower_message:
        return "请求超时"
    # 可以根据实际遇到的错误添加更多翻译规则
    # ...
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    if isinstance(exc, HTTPException):  # 如果捕获到的是 FastAPI 的 HTTPException
        ...
        if (
            ...
            error_content = ErrorResponse(
                message=str(detail), type=type(exc).__name__, code=str(status_code)
            ).model_dump()
            return JSONResponse(
                status_code=status_code, content=error_content, headers=exc.headers
            )
        else:
    ...
    if isinstance(exc, HTTPException) and exc.headers and "Location" in exc.headers:
        logger.info(
            f"全局异常处理器：HTTPException (状态码 {exc.status_code}) 包含 Location 头，执行重定向到 {exc.headers['Location']}"
        )
    ...
    if isinstance(exc, HTTPException):
        error_content = ErrorResponse(
            message=str(exc.detail), type=type(exc).__name__, code=str(exc.status_code)
        ).model_dump()
        return JSONResponse(
            status_code=exc.status_code, content=error_content, headers=exc.headers
        )

    # --- 对于所有其他未处理的 Exception ---
    error_content = ErrorResponse(
        message=translate_error(str(exc)),
