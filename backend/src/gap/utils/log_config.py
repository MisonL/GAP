...
from logging.handlers import (  # 日志文件轮转处理器
    RotatingFileHandler,
...
try:
    ...
    try:
        ...
        print(
            f"警告: 无法在项目目录创建日志文件夹，将使用系统临时目录: {log_dir}"
        )  # 打印警告
    except Exception as temp_e:  # 如果在临时目录创建也失败
        log_dir = None  # 禁用文件日志
        print(
            f"警告: 尝试在项目目录和临时目录创建日志文件夹均失败: {temp_e}。文件日志记录将被禁用。"
        )  # 打印最终警告
    ...
    try:
        os.makedirs(log_dir, exist_ok=True)
        print(
            f"警告: 在项目目录创建日志文件夹失败 ({e})，将尝试使用当前工作目录: {log_dir}"
        )  # 打印警告
    except Exception as final_e:  # 如果所有尝试都失败
        log_dir = None  # 禁用文件日志
        print(
            f"警告: 尝试在多个位置创建日志目录均失败: {final_e}。文件日志记录将被禁用。"
        )  # 打印最终警告
...
# 定义普通模式下的简洁日志格式
LOG_FORMAT_NORMAL = (
    "%(key_fmt)s%(request_type)s%(model_fmt)s%(status_code)s: %(message)s"
)
        ...
        try:
            ...
            if app_log_file:  # 检查应用日志文件路径是否有效
                try:
                    ...
                    print(
                        f"警告: 无法创建应用日志处理程序 ({app_log_file}): {e}"
                    )  # 打印警告
            ...
            if error_log_file:  # 检查错误日志文件路径是否有效
                try:
                    ...
                    print(
                        f"警告: 无法创建错误日志处理程序 ({error_log_file}): {e}"
                    )  # 打印警告
...
def format_log_message(
    ...
    request_type = extra.get(
        "request_type", ""
    )  # 请求类型 (例如 'chat', 'list_models')
...
def cleanup_old_logs(max_days: int = LOG_CLEANUP_DAYS):
    ...
    try:
        ...
        if deleted_count > 0:  # 如果删除了文件
            print(
                f"日志清理完成，共删除 {deleted_count} 个超过 {max_days} 天的日志文件。"
            )
        else:  # 如果没有删除文件
