...
from typing import AsyncGenerator, Callable, Awaitable # 异步生成器类型提示, Callable, Awaitable
from asyncio import TimeoutError # 异步超时错误
from fastapi.staticfiles import StaticFiles # 静态文件服务
from fastapi.responses import Response, FileResponse # 导入 Response 和 FileResponse
from fastapi.security import OAuth2PasswordRequestForm # 导入 OAuth2PasswordRequestForm
...
# --- 第三方库导入 ---
from fastapi import FastAPI, HTTPException, Request, Response, Depends, status, Form # FastAPI 框架核心类, HTTPException, Request, Response, Depends, status, Form
...
from sqlalchemy.ext.asyncio import async_sessionmaker, AsyncSession  # 导入异步会话工厂和异步会话类
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    # 创建共享的异步 HTTP 客户端实例，并设置显式超时 (例如，总超时30秒，连接超时10秒)
    timeout_config = httpx.Timeout(30.0, connect=10.0)
    http_client = httpx.AsyncClient(timeout=timeout_config) 
    logger.info(f"共享 HTTP 客户端已初始化，超时设置为: connect={timeout_config.connect}, read/write/pool={timeout_config.read}") # 使用正确的属性名
    ...
    async with app.state.AsyncSessionFactory() as db_session_for_check: # 使用异步会话工厂获取会话
        try:
    ...
    # --- 加载模型限制配置 ---
    config.MODEL_LIMITS = load_model_limits() # 调用 config 模块中的函数加载 JSON 文件，并将返回值赋给 config.MODEL_LIMITS
...
@app.middleware("http")
async def add_permissions_policy_header(request: Request, call_next: Callable[[Request], Awaitable[Response]]):
...
async def serve_login_page():
    return FileResponse("/app/frontend/dist/login.html")

# 处理POST请求进行认证
@app.post("/login", status_code=204) # 登录路由，返回 204 No Content
async def login_for_access_token(
    password: str = Form(...),  # 直接接收password字段，无需username
    key_manager: APIKeyManager = Depends(lambda: app.state.key_manager) # 获取 Key 管理器实例
):
    """
    处理用户登录并返回访问令牌。
    """
    ...
    if not is_valid:
        logger.warning(f"登录失败: 无效的 API Key '{api_key[:8]}...'") # 记录警告日志
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, # 401 未授权状态码
            detail="无效的 API Key", # 错误详情
            headers={"WWW-Authenticate": "Bearer"}, # 认证头部
        )

    # 如果 API Key 有效，则创建访问令牌
    access_token_expires = timedelta(minutes=config.ACCESS_TOKEN_EXPIRE_MINUTES) # 设置 token 过期时间
    access_token = create_access_token(
        data={"sub": api_key, "admin": key_manager.is_admin_key(api_key)}, # token 数据，包含 sub 和 admin 状态
        expires_delta=access_token_expires # 过期时间
    )
    
    response = Response(status_code=status.HTTP_204_NO_CONTENT) # 创建 204 响应
    response.headers["X-Access-Token"] = access_token # 将 token 放在自定义头部
    logger.info(f"API Key '{api_key[:8]}...' 登录成功，已颁发访问令牌。") # 记录成功日志
...
# 将所有静态文件和SPA路由交给前端处理
# 使用Docker容器内的正确路径
# 设置html=True让StaticFiles自动处理SPA路由fallback
app.mount("/", StaticFiles(directory="/app/frontend/dist", html=True), name="frontend")
logger.info("已挂载 Vue SPA 静态文件目录 (/app/frontend/dist) 到根路径 /。")
# --- 打印所有已注册的路由 ---
