<template>
  <div class="config-view">
    ...
    <!-- 内存模式警告 -->
    <div
      v-if="configInfo?.is_memory_mode"
      ...
      <div class="flex">
        ...
        <div class="ml-3">
          <p class="text-sm text-yellow-700">
            <strong>警告：</strong>
            <span>
              当前运行在纯内存模式下，所有配置修改仅在当前会话中有效，重启服务后将恢复原始配置。
            </span>
          </p>
        </div>
      </div>
    ...
    <div class="bg-white shadow rounded-lg p-6 mb-6">
      ...
      >
        ...
        <template v-if="configInfo.is_admin">
          <div class="space-y-4">
            ...
            <div>
              ...
              <p class="mt-1 text-sm text-gray-900">
                {{ configInfo.context_storage_mode }}
              </p>
            </div>
          ...
          <div class="space-y-4">
            <div>
              ...
              <p class="mt-1 text-sm text-gray-900">
                {{ configInfo.max_requests_per_minute }}
              </p>
            </div>
            ...
            <div>
              ...
              <p class="mt-1 text-sm text-gray-900">
                {{ configInfo.max_requests_per_day_per_ip }}
              </p>
            </div>
            ...
            <div>
              ...
              <p class="mt-1 text-sm text-gray-900">
                {{ configInfo.web_ui_passwords_count }}
              </p>
            </div>
            ...
            <div>
              ...
              <p class="mt-1 text-sm text-gray-900">
                {{ configInfo.gemini_api_keys_count }}
              </p>
            </div>
          </div>
        </template>
        ...
        <template v-else>
          ...
          <!-- 用户自己的key信息 -->
          <div
            v-if="configInfo.user_key_info"
            class="space-y-4"
          >
            <div>
              <label class="block text-sm font-medium text-gray-700">当前密钥</label>
              <p class="mt-1 text-sm text-gray-900">
                {{ configInfo.user_key_info.key_string }}
              </p>
            </div>
            ...
            <div>
              ...
              <p class="mt-1 text-sm text-gray-900">
                {{ configInfo.user_key_info.usage_count || 0 }}
              </p>
            </div>
          </div>
        </template>
    ...
    <!-- 配置修改卡片（仅管理员和内存模式） -->
    <div
      v-if="configInfo?.is_admin && configInfo?.is_memory_mode"
    ...
    >
      ...
      >
        ...
        <div>
          <label
            for="max_requests_per_day_per_ip"
            class="block text-sm font-medium text-gray-700"
          >
            每日每IP最大请求数
          </label>
      ...
      <div
        ...
        :class="updateResult.success ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'"
      >
    ...
    <!-- 普通用户提示 -->
    <div
      v-if="!configInfo?.is_admin"
      class="bg-gray-50 border-l-4 border-gray-400 p-4"
    >
      <div class="flex">
        <div class="flex-shrink-0">
          <svg
